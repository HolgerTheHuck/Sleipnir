...
- `SleipnirSseClient`: events-only over `text/event-stream` with `Last-Event-Id` resume, `IDisposable`.
...
- `SleipnirCall`: fluent builder — `.Init("Controller", "Method").With(args).Named("id").Exposes("$", "alias").WithAlias("@alias").ToRequest()`. An unresolved `@alias` fails the call — there is no implicit default fallback in v1. The `Exposes` JSON path is **result-relative** (`$` = whole result, `$.Property`, `$[0].Id`); there is no `$.data` envelope level.
...
`SleipnirCore.Tracing.SleipnirTracing` instruments the engine with an always-on `ActivitySource` named `"Sleipnir"` (public class so `ActivitySourceName` is reachable from the optional `Sleipnir.Telemetry` package; all other members are `internal`). Instrumentation lives **directly in `SleipnirInvoker`** at three sites (single-call `InvokeDi(SleipnirRequest)`, batch dispatcher `InvokeDi(IEnumerable<SleipnirRequest>)`, and per-request `ExecuteAuthorized`) — not as an `ISleipnirInterceptor`, because the batch path bypasses the interceptor pipeline. It is cost-neutral: `StartActivity` returns `null` without a listener, and it is not DI-registered, so the `AddSleipnir` registration path needs no synchronization around it. `Activity.RecordException` is unavailable in a net8.0 class library — `SleipnirTracing.RecordException` sets `exception.type`/`exception.message`/`exception.stacktrace` tags directly. `SleipnirServer` does **not** reference `Sleipnir.Telemetry`; consumers opt in via `AddSleipnirTelemetry` (or their own `AddOpenTelemetry().WithTracing(b => b.AddSource("Sleipnir"))`). Tracing/telemetry tests rely on the process-global `ActivityListener`, so they share the `sleipnir-tracing` xUnit collection and use a test-harness activity to isolate their spans from parallel invoker-based tests.
