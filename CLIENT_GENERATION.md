...
> export + drift-check (`Sleipnir.Server.Codegen`) and the client-side Roslyn source generator
> (`Sleipnir.Generator`), the regen flow, CI, config, and troubleshooting. This document is the
...
Sleipnir's code-first promise — *the C# classes are the contract, no IDL, no `.proto`* — is a
**server-side** advantage. The .NET client gets a typed fluent builder (`SleipnirCall` /
`ISleipnirClient`), but every **non-.NET client** (a browser TypeScript frontend, a Node service,
...
That is the strongest adoption objection against Sleipnir for polyglot shops, and it is
addressable **without giving up the code-first principle**: the runtime discovery payload
(`GET /api/sleipnir/discovery`) already carries the full contract as structured JSON — controllers,
...
- The reusable logic already exists in TypeScript — the type-name tables and static binding
  analysis in `SleipnirDeveloperUi/src/lib/utils/{params,dependencyCheck}.ts`, the wire + discovery
  types and the fluent builder in `clients/ts/` (`sleipnir-client`). A TS core reuses these directly;
...
- The non-.NET clients are the gap; TS/JS are the highest-value emitters first. C# already has a
  real client (`Sleipnir.Client`) — a generated typed C# surface is an ergonomics win, not a gap-closer.
...
The generator lives in a new package [`clients/codegen/`](clients/codegen/) (`sleipnir-codegen`),
depends on `sleipnir-client` (`file:../ts`) for the wire types, the fluent builder, and
`SleipnirRestClient.discover()`. It ships a CLI: `npx sleipnir-gen`.
...
|---|---|---|---|---|
| TS / JS | npm / vite / npx | native (`sleipnir-codegen` devDependency) | A | the TS core |
...
- **Two C# emitters means a parity gate.** The TS `--lang cs` emitter (DevUI/CI) and the Roslyn
  generator (build) must produce equivalent C# for the same `contract.sleipnir.json`. A conformance
...
const order = await client.order.getById(42);          // Promise<SleipnirResponse<Order | null>>
...
fluent builder (`SleipnirCall.batch([...])` from `sleipnir-client`) remains available as an escape hatch.
...
The discovery payload is the generator's input and — for anyone who wants to implement a Sleipnir
...
client (or server) without reading Sleipnir's source. The no-drift guarantee is enforced by a
conformance gate (`SleipnirTests/Integration/DiscoveryContractTests.cs`) that rebuilds the discovery
...
- **Increment 1 (done)** — `clients/codegen` core (extracted + consolidated) + **TS** and
  **JS** emitters + `sleipnir-gen` CLI + DevUI `CodegenPage` refactor (thin wrapper over the core) +
...
- **Increment 2 (done)** — **C#** and **Python** Node emitters (`csTypeOf` / `pyTypeOf` tables in
  ...
  `SleipnirCall.Init(...).Exposes(...).WithAlias(...)` surface. Python: `dataclasses` + async over
...
- **Delivered (.NET-native C# trio — was the v1.1 endgame, pulled forward)** — **.NET-native C#:
  ...
  1. **Export (server side) — done.** `Sleipnir.Server.Codegen` (net8.0 Exe, NuGet `Sleipnir.Server.Codegen`)
     loads the built server assembly in its own process, reflects the `[SleipnirController]` types
     (scoped to the server output dir, not AppDomain-wide), builds a `SleipnirInvoker`, calls
     `GetDiscoveryInfo()`, sorts controllers by name for determinism, and serializes with the same
     `DiscoverySerialization.Options` `/api/sleipnir/discovery` uses — writing `contract.sleipnir.json`.
     No running HTTP server, no Node. `build/Sleipnir.Server.Codegen.targets` runs it `AfterTargets="Build"`.
  2. **Generate (client side) — done.** `Sleipnir.SourceGenerator` (netstandard2.0, NuGet `Sleipnir.Generator`)
     is a Roslyn `IIncrementalGenerator` that reads `contract.sleipnir.json` via `AdditionalFiles` and
     emits C# stubs **into the compilation**. NuGet `PackageReference`, interface break → build fails.
     The C# emission logic lives in `Sleipnir.Codegen.Core` (a pure C# port of the TS emitter) and is
     **linked into the generator** (not subprocessed — the Roslyn analyzer load context cannot
     resolve a ProjectReference dep); the standalone `Sleipnir.Codegen.Core` project remains as the
     parity-gate + compile-gate target. `build/Sleipnir.Generator.props` marks the contract as an
     AdditionalFile. Diagnostics `SLEIPNIR001` (shape violation) / `SLEIPNIR002` (emit failure); the
  ...
  3. **Drift check (mandatory component) — done, and the failure path is now automated.** The
     `Sleipnir.Server.Codegen` target regenerates the discovery at build time and **fails the build**
     (exit 1) if the committed JSON differs; `SLEIPNIR_REGEN_GOLDEN=1` regenerates instead. Without
     it this is exactly the `wsdl.exe` trap. Story 01 is the first consumer
     (`stories/01-n-plus-one-screen/contract.sleipnir.json` committed). `SleipnirTests/Integration/
  ...
  Plus a **parity gate — done**: `Sleipnir.Codegen.Core.EmitClient` vs the committed
  `clients/codegen/test/snapshots/story01.cs/SleipnirGenerated.cs` byte-for-byte, plus a spawn-`dotnet
  build` compile gate against SleipnirClient (`SleipnirTests/Unit/Core/CsCodegenParityTests.cs`). And a
  **generator diagnostic gate — done**: `SleipnirTests/Unit/Core/SourceGeneratorDiagnosticsTests.cs`
  exercises the `CodegenSeam.TryEmit` mapping (valid → source, no diagnostic; shape violation →
  SLEIPNIR001; malformed JSON → SLEIPNIR002; source suppressed on both failure paths). This gives .NET
...
- **CI publishing** — publish `sleipnir-client` and `sleipnir-codegen` to npm; ship the Roslyn
  generator as a NuGet package, from `.github/workflows/` (today both Sleipnir npm packages are local
  `file:` dependencies).
- **Deferred cleanups** — unify `BatchCodegen`'s snippet renderer into `sleipnir-codegen/snippets`;
  ...
  `SleipnirDeveloperUi/src/lib/utils/params.ts`, sourced from `sleipnir-codegen`'s scalar tables.)
