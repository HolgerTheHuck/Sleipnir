...
Sleipnir ships an **opt-in** JSON-RPC 2.0 adapter so any existing JSON-RPC client
ecosystem can drive a Sleipnir server as-is. The adapter is an adoption lure: callers
start with the JSON-RPC tooling they already have and, once they want chaining,
batch execution modes, or binary out-of-band, graduate to the native Sleipnir wire.

The adapter maps JSON-RPC requests onto the **same** `SleipnirInvoker` that the REST,
...
app.MapSleipnir();                   // or app.MapSleipnirEndpoints("/api/sleipnir", enableJsonRpcCompat: true);
```
...
`MapSleipnir()` reads `SleipnirOptions.EnableJsonRpcCompat` and forwards it; if you call
`MapSleipnirEndpoints` directly, pass `enableJsonRpcCompat: true`.
...
The HTTP envelope is **envelope-at-200** like the native Sleipnir REST transport: every
...
A `method` with no dot is `-32600 Invalid Request`. Sleipnir dispatches by name only
(`{Controller}_{Method}`, no parameter-based overload resolution), so each JSON-RPC
`method` maps to exactly one Sleipnir method.
...
`byte[]` parameters are **not** bound positionally (a documented Sleipnir constraint); use
...
a string id as a string. Internally, the id is also stringified for Sleipnir's correlation
and tracing (`sleipnir.request_id`).
...
Sleipnir's logical body codes are mapped onto the JSON-RPC error ranges:

| Sleipnir code | JSON-RPC code | Meaning                          |
|------------|---------------|----------------------------------|
| 400, 422   | `-32602`      | Invalid params (binding/validation) |
| 404 (routing — Controller/Method not found) | `-32601` | Method not found |
| 404 (business — `SleipnirResults.NotFound`)    | `-32000` | Server error |
...
The **Routing vs. Business 404** distinction is the one subtlety. Sleipnir's 404 is
ambiguous: it is produced both when a controller/method does not exist (framework, in
the authorization pre-pass) and when a controller returns `SleipnirResults.NotFound`
...
When the Sleipnir error carries a structured payload (e.g. a `ProblemDetails` body from
`SleipnirResults.Error(ProblemDetails)`), that payload becomes `error.data`. Otherwise,
`SleipnirError.details` (a diagnostic string, present in Development) is used. If neither
exists, `data` is omitted.

---

## Capability methods — `sleipnir.*`

Two reserved methods bridge to Sleipnir's discovery and advertise the strengths that the
compat mode does **not** expose:

### `sleipnir.discover`

Returns the full Sleipnir discovery metadata (the same object as
`GET /api/sleipnir/discovery`) as the JSON-RPC `result`. Types are structured,
...
```json
{ "jsonrpc": "2.0", "method": "sleipnir.discover", "id": 1 }
→ { "jsonrpc": "2.0", "result": { /* DiscoveryInfo */ }, "id": 1 }
```

### `sleipnir.capabilities`

Returns a static manifest of the native Sleipnir features, so a JSON-RPC client knows
...
## Protocol differences: Sleipnir wire vs. JSON-RPC 2.0

The two share a common basis — **transmit parameters as JSON, write a result (or error)
back** — but Sleipnir's wire is richer because it targets a single, code-first .NET engine.
JSON-RPC 2.0 is the *lowest common denominator*; the differences are logical, not
arbitrary:

| Aspect | JSON-RPC 2.0 | Native Sleipnir wire | Why Sleipnir differs |
|--------|--------------|-------------------|-------------------|
| **Parameters** | `params` is a structured JSON value (object/array) the server consumes natively. | `params` is a native JSON array of `[{parameterName, data, num}]` where each `data` is itself a native JSON value (number, string, object, …). | Same shape as JSON-RPC at the value level: Sleipnir adopted native `params` pre-1.0 (the earlier double-encoded `stringData` was removed). The engine binds each `data` to its C# type directly and rewrites the array in place for `@alias` substitution. The compat adapter is therefore identity at the `params` level — it only reshapes `Controller.Method` routing and the envelope. |
| **Batch shape** | A plain top-level JSON array; responses in a plain array. | `SleipnirMultiRequest` envelope `{requests:[…], mode}`. | Sleipnir carries the **execution mode** (Parallel / Serial / auto-detect topological) in the envelope so the server picks the batch strategy per call. |
...
| **Result vs. error** | `result`/`error` mutually exclusive; `id` always present (null if unknown). | `SleipnirResponse` always carries a logical `code` (HTTP-like) with `data` and optional `error`; `isSuccess` is derived. | Sleipnir unifies success and *business* error into one envelope so a domain `NotFound` (404) is a real payload, not a transport failure — and so dependency propagation can name *why* a provider failed. |
...
| **Discovery / MEX** | Not part of the spec. | First-class `GET /api/sleipnir/discovery` returning full type metadata inferred from signatures. | Code-first means the contract *is* the C# classes; discovery makes it consumable for codegen and the built-in DevUI without an IDL. |
...
directly) is simpler than the double-encoded `stringData` Sleipnir originally shipped
...
Sleipnir removed it before 1.0: `params` is now a native JSON array with native `data`
...
These are deliberately native-only — graduate to the Sleipnir wire to use them:
...
* **Method must be `Controller.Method`.** JSON-RPC method names without a dot are
  `-32600`. Sleipnir's dotted controller namespaces are supported (split at the last dot).

## Graduating to native

Switching a caller from the compat endpoint to the native wire (`POST /api/sleipnir/json`,
`POST /api/sleipnir/json/multi`, or the WebSocket/SignalR transports) unlocks chaining,
mode selection, binary, and the full discovery-driven client (`SleipnirCall`, the TS/JS
client). The discovery metadata from `sleipnir.discover` is identical to
`GET /api/sleipnir/discovery`, so codegen against either produces the same client surface.

---

## Implementation map

* `SleipnirRest/JsonRpc/JsonRpcAdapter.cs` — pure bidirectional translator (request →
  `SleipnirRequest`, `SleipnirResponse` → JSON-RPC, error-code map, `sleipnir.capabilities`).
  No transport or DI dependency; unit-tested in isolation.
* `SleipnirRest/JsonRpc/JsonRpcDispatcher.cs` — orchestration: reads the body, dispatches
  capability methods, calls `ISleipnirCore.InvokeDi` (Parallel), assembles single/batch
  responses, applies the 200/204 envelope rules.
* `SleipnirRest/JsonRpc/JsonRpcModels.cs` — `JsonRpcRequest` DTO + `ParsedRpcItem`
  working struct.
* `SleipnirHub/Extensions/SleipnirOptions.cs` — `EnableJsonRpcCompat` option (default false).
* `SleipnirRest/SleipnirEndpointExtensions.cs` — `enableJsonRpcCompat` param + `POST /jsonrpc`.
* Tests: `SleipnirTests/Unit/Rest/JsonRpcAdapterTests.cs` (translation),
  `SleipnirTests/Integration/JsonRpcTransportTests.cs` (end-to-end over Kestrel).