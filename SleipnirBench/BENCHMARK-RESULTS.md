# Sleipnir Performance Benchmarks
...
- **HTTP/1.1-Port** für REST, Sleipnir-REST, Sleipnir-WebSocket (`/sleipnirws`) und Sleipnir-SignalR (`/sleipnirhub`).
...
| **Sleipnir REST** | HTTP/1.1 + JSON | System.Text.Json (Sleipnir-Wrapper) | 1 Batch |
| **Sleipnir WebSocket** | RFC 6455 + JSON-Text-Frames | System.Text.Json | 1 Batch |
| **Sleipnir SignalR** | WebSocket + MessagePack | MessagePack (binär) | 1 Batch |

> **Vorab, die Kernthese:** Sleipnir bündelt N Aufrufe in **einen** Roundtrip. Bei 100 Aufrufen schlägt das natives REST (sequenziell **und** parallel) sowie natives gRPC (sequenziell) um **Faktor 16–60**. Bei Einzelaufrufen ist gRPC dank Protobuf-Binärformt tendenziell etwas kompakter; Sleipnir SignalR (MessagePack) hält hier dagegen. Die Dependency-Chain spare **Roundtrips (3 → 1)**, aber auf localhost überdeckt die Service-Latenz diesen Vorteil — er wirkt erst über hoher Netzwerklatenz.
...
| `Sleipnir REST: GetAllCustomers` | 480.2 µs | 163.81 KB |
| `Sleipnir REST: GetCustomerById` | 226.9 µs | 11.13 KB |
| `Sleipnir WebSocket: GetAllCustomers` | 589.5 µs | 253.36 KB |
| `Sleipnir WebSocket: GetCustomerById` | 212.9 µs | 10.41 KB |
| `Sleipnir SignalR: GetAllCustomers` | 342.7 µs | 66.58 KB |
| `Sleipnir SignalR: GetCustomerById` | 183.5 µs | 6.31 KB |

### Lesart

- **Einzelner GetById-Call:** Sleipnir **SignalR (183 µs)** und **WebSocket (213 µs)** sind die schnellsten Kanäle — die persistente Verbindung amortisiert sich schon beim ersten Call (kein HTTP/1.1-Header-Overhead pro Aufruf). Sleipnir REST (227 µs) schlägt natives REST (504 µs) deutlich, weil der Sleipnir-Endpoint minimaler ist als die MVC-Controller-Pipeline. gRPC (342 µs) liegt dazwischen.
- **GetAll (100 Datensätze):** Natives REST (357 µs) und gRPC (348 µs) sind hier knapp schneller als Sleipnir REST (480 µs) / WebSocket (590 µs) — der JSON-Envelope-Overhead der Sleipnir-Response wird bei großem Payload sichtbar. **Sleipnir SignalR (343 µs, MessagePack-Binär)** hält dagegen auf gRPC-Niveau.
- **Allokation:** gRPC allokiert am wenigsten (Protobuf-Binär, vordefinierte Message-Typen). Sleipnir SignalR ist der allokationseffizienteste Sleipnir-Kanal. Sleipnir REST/WebSocket allokiert beim `GetAll` deutlich mehr (163 / 253 KB) — der JSON-Text-Envelope inkl. Discovery-Metadaten-Felder ist hier der Nachteil.

> **Ehrlicher Nachteil:** Bei einzelnen Aufrufen mit großem Payload ist natives gRPC (und z. T. natives REST) kompakter und teils schneller als Sleipnirs JSON-Kanäle. Sleipnir SignalR (MessagePack) gleicht das weitgehend aus.

---

## 2. Batch Benchmarks — 100 Aufrufe

### Szenario: 100× `GetCustomerById` — nativ (REST/gRPC, je sequenziell + parallel, also 100 Roundtrips) vs. Sleipnir-Batch (1 Roundtrip, 100 Aufrufe serverseitig gebündelt, je parallel + serial).
...
| `Sleipnir REST: 100x batch parallel` | 876.5 µs | 0.05 | 513.76 KB |
| `Sleipnir REST: 100x batch serial` | 1.069 ms | 0.06 | 641.55 KB |
| `Sleipnir WebSocket: 100x batch parallel` | 918.4 µs | 0.06 | 792.22 KB |
| `Sleipnir WebSocket: 100x batch serial` | 1.148 ms | 0.07 | 920.06 KB |
| `Sleipnir SignalR: 100x batch parallel` | 555.3 µs | 0.03 | 335.59 KB |
| `Sleipnir SignalR: 100x batch serial` | 1.429 ms | 0.09 | 463.44 KB |

### Lesart

- **Sleipnir-Batch schlägt natives REST um Faktor 15–30:** 100 Aufrufe in **einem** Roundtrip (876 µs REST-parallel) statt 100 Roundtrips (16,5 ms REST-sequenziell / 11,9 ms -parallel). **Sleipnir SignalR parallel (555 µs) ist ~30× schneller als REST sequenziell und ~21× schneller als REST parallel.**
- **Auch gRPC-parallel geschlagen:** gRPC-parallel (3,1 ms, HTTP/2-Multiplexing) ist gut — aber Sleipnir SignalR-parallel (555 µs) ist **~5,6× schneller**, Sleipnir REST-parallel (876 µs) **~3,6× schneller**. Der Roundtrip-Bündelungs-Vorteil überkompensiert Protobufs Binär-Vorteil.
...
- **Serial vs. Parallel (Sleipnir):** Parallel ist erwartungsgemäß schneller als Serial (serverseitig `Task.WhenAll` vs. sequenzielle `foreach`). SignalR-serial (1,43 ms) ist langsamer als REST/WS-serial — die serial-Mode-Ausführung wirkt sich über die MessagePack-Hub-Pipeline stärker aus.
- **Allokation:** Sleipnir SignalR parallel allokiert am wenigsten (336 KB). Sleipnir WS-serial (920 KB) und REST-sequenziell (936 KB) sind vergleichbar. Die JSON-Kanäle sind bei Serial allokationsintensiver.

> **Kernvorteil Sleipnir:** Der Batch-Modus reduziert 100 Roundtrips auf 1. Genau hier — viele Calls pro Logikschritt — ist Sleipnirs Mehrwert am größten, und zwar über **alle** drei Kanäle, nicht nur SignalR.

---

## 3. Dependency-Chain Benchmarks

### Szenario: Klassisches N+1-Muster — `AddCustomer` → `GetCustomerById` → `GetOrdersByOrderId`. REST und gRPC brauchen **3 sequentielle Roundtrips** (gRPC hat kein serverseitiges Chaining). Sleipnir schickt **1 Batch** mit `@alias`-Dependency-Mapping (Serial-Mode, serverseitig verkettet).
...
| `Sleipnir: 3 sequential single calls (no chaining)` | 206.2 ms | 1.00 | 34.55 KB |
| `Sleipnir: 1 batch call (dependency chain, 3 reqs)` | 208.1 ms | 1.01 | 42.18 KB |
| `Sleipnir WebSocket: 1 batch call (dependency chain)` | 207.9 ms | 1.01 | 42.66 KB |

### Lesart — die ehrliche Geschichte

- **Auf localhost ist kein Speedup messbar.** Alle Ansätze liegen bei **~207 ms** — und das liegt **nicht** an Sleipnir, sondern am simulierten `Task.Delay(200 ms)` in `AddCustomer`. Diese Service-Latenz dominiert die gesparten Roundtrips komplett. Die alte (veraltete) Dokumentation behauptete „78 µs / 2646× Speedup" — das war **physikalisch unmöglich** und ist hier korrigiert.
- **Der echte Sleipnir-Vorteil ist strukturell, nicht lokal:** 3 Roundtrips → 1 Roundtrip. Über eine Netzwerklatenz von z. B. 50 ms pro Roundtrip spart Sleipnir hier **100 ms** (2 Roundtrips × 50 ms) ein — der `Task.Delay(200 ms)` bleibt, aber die Latenz reduziert sich von ~300 ms + 200 ms auf ~50 ms + 200 ms. **Der Roundtrip-Vorteil wirkt erst über hoher Netzwerklatenz**, nicht auf localhost. Genau das ist die ehrliche Aussage.
- **Allokation — ein echter Nachteil:** Sleipnirs Batch-Chain (42 KB) allokiert **mehr** als REST (30 KB) und deutlich mehr als gRPC (52 KB ist hier allerdings durch Protobuf-Message-Objekte bedingt). Die JSON-Alias-Auflösung + Envelope + Dependency-Resolver verbrauchen Extra-Speicher. Das ist ein fairer, dokumentierter Nachteil.
- **Sleipnir 3-sequential-no-chaining (34 KB) vs. Sleipnir 1-batch-chain (42 KB):** Auch ohne Chaining ist die 3-Call-Sequenz allokationseffizienter als der Batch — der Batch-Envelope schlägt hier zu Buche. Dafür hat der Batch eben nur 1 Roundtrip.

> **Fazit Dependency-Chain:** Sleipnirs Chaining ist ein **Architektur-Vorteil** (weniger Roundtrips, eine atomare Logikeinheit, serverseitig korreliert), kein lokaler Performance-Wunder. Über Netzwerklatenz wird er zum echten Speedup; auf localhost bleibt er neutral bei höherer Allokation.
...
### ⚠️ Nachteile Sleipnir

1. **JSON-Text-Envelope bei großem Payload.** Bei `GetAllCustomers` sind Sleipnir REST (480 µs / 164 KB) und WebSocket (590 µs / 253 KB) langsamer und allokationsintensiver als natives REST (357 µs / 86 KB) und gRPC (348 µs / 57 KB). SignalR (MessagePack) gleicht das aus — aber REST/WS-Kanäle nicht.
...
3. **Höhere Allokation beim Batch/Chain.** Sleipnir-Batch (42 KB) > REST-sequenziell (30 KB). Alias-Resolver + Envelope kosten Extra-Speicher.
...
dotnet run -c Release --project SleipnirBench/SleipnirBench.csproj -- --filter "*Ws*"
```
