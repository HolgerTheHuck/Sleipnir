...
command-oriented web APIs on .NET 8+. Three transports share one `ISleipnirClient`
surface, so the same call runs over REST, WebSocket, or SignalR.

- **Transports:** `SleipnirRestJsonClient` (`HttpClient`, `IDisposable`),
  `SleipnirWebSocketClient` (persistent `ClientWebSocket`, auto-reconnect, `IAsyncDisposable`),
  `SleipnirSignalrClient` (SignalR hub, MessagePack, `IAsyncDisposable`).
