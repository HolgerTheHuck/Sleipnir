...
namespace SleipnirCommon;

/// <summary>
/// Server-seitiger Write-Only-Converter für <see cref="SleipnirResponse"/> (REST + WS).
/// Schreibt <see cref="SleipnirResponse.DataBytes"/> via <c>Utf8JsonWriter.WriteRawValue</c>
...
/// <see cref="SleipnirResponse.Data"/> gesetzt (Legacy-/ProblemDetails-Pfad), fällt der
...
/// Server parst keine <see cref="SleipnirResponse"/> (nur Requests); der Client nutzt den
/// shared <c>SleipnirResponseParser</c> bzw. die Default-Deserialisierung.
/// </remarks>
public sealed class SleipnirResponseJsonConverter : JsonConverter<SleipnirResponse>
{
    public override void Write(Utf8JsonWriter writer, SleipnirResponse value, JsonSerializerOptions options)
    {
        ...
        // error: SleipnirError? — rekursiv (respektiert [JsonPropertyName]-Attribute der SleipnirError).
    ...
    public override SleipnirResponse Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        => throw new NotSupportedException(
            "SleipnirResponseJsonConverter ist Write-Only — der Server parst keine SleipnirResponse.");
}