...
{
    public class SleipnirInvoker : ISleipnirCore
    {
        ...
        private readonly SleipnirDiscoveryService _discoveryService;
        private readonly IServiceScopeFactory _serviceScopeFactory;

        private readonly ILogger<SleipnirInvoker> _logger;
        private readonly List<ISleipnirInterceptor> _interceptors;

        /// <summary>
        /// Hotfix 1.1.1: Optionaler Policy-Evaluator für Batch-Pfad. Wird von AddSleipnir
        /// (SleipnirHub) gesetzt, wenn IAuthorizationService verfügbar ist — erlaubt
        /// CheckAuthorisation, Policies auch im Batch-Pre-Pass auszuwerten (nicht nur
        /// im Single-Call-Pfad via SleipnirAuthorizationInterceptor). SleipnirCore bleibt frei
        ...
        /// (SleipnirError.Details) befüllt. In Produktion auf false belassen, um keine
        /// internen Informationen an Clients preiszugeben. Wird durch AddSleipnir anhand
        /// von SleipnirOptions.EnableDetailedErrors bzw. IHostEnvironment.IsDevelopment() gesetzt.
        /// </summary>
        ...
        /// greifen nicht). string/byte[] ausgenommen. Wird über AddSleipnir aus
        /// SleipnirOptions gesetzt; der Invoker-Default schützt auch einen nackten `new SleipnirInvoker()`.
        /// </summary>
        ...
        /// Parameter. Siehe DEPENDENCY_BINDING.md. Wird über AddSleipnir aus SleipnirOptions
        /// gesetzt; der Invoker-Default (Weak) gilt auch für einen nackten new SleipnirInvoker().
        /// </summary>
        ...
        /// nicht breaking). Wird über AddSleipnir aus SleipnirOptions gesetzt. Siehe
        /// SECURITY.md.
        /// </summary>
        ...
        public SleipnirInvoker(IServiceScopeFactory serviceScopeFactory, ILogger<SleipnirInvoker> logger, IEnumerable<ISleipnirInterceptor>? interceptors = null)
        {
            _logger = logger;
            _serviceScopeFactory = serviceScopeFactory;
            _discoveryService = new SleipnirDiscoveryService(_routeHandlers);
            _interceptors = interceptors?.ToList() ?? new List<ISleipnirInterceptor>();
        }
        ...
        {
            var controllerAttr = GetAttribute<SleipnirControllerAttribute>(controllerType);
            ...
            lock (_registrationLock)
            {
                ...
                // (SleipnirDiscoveryService derives from _routeHandlers, so the cache only ever
                ...
                foreach (var methodInfo in controllerType.GetMethods())
                {
                    ...
                    var invokeInfo = new InvokeInfo()
                    {
                        ...
                        AuthoriseAttribute = GetAttribute<SleipnirAuthoriseAttribute>(methodInfo)
                            ?? GetAttribute<SleipnirAuthoriseAttribute>(controllerType),
                        AnonymousAttribute = GetAttribute<SleipnirAnonymousAttribute>(methodInfo),
        ...
        // WARN (Hotfix 1.1.1): ISleipnirBatchInterceptor / SleipnirOptions.BatchInterceptors werden
        // aktuell NICHT aufgerufen — der Batch-Pfad geht direkt in ExecuteInParallel/etc.
        // ohne Batch-Interceptor-Pipeline. User-Interceptors aus SleipnirOptions.Interceptors
        ...
        public async Task<IEnumerable<SleipnirResponse?>> InvokeDi(
            ...
            CancellationToken ct = default)
        {
            ...
            // dieser Check schützt direkte In-Process-Aufrufer (Tests, ISleipnirCore-Konsumenten).
            ...
            // Batch-Metrics (sleipnir.batch.fan_out/count) — kostenneutral ohne MetricReader.
            SleipnirMetrics.RecordBatch(requestList, mode);
            ...
            {
                batchActivity?.SetTag("sleipnir.batch.mode", "DependencyBatches");
        ...
        public async Task<SleipnirResponse?> InvokeDi(SleipnirRequest request, HttpContext? context, CancellationToken ct = default)
        {
            // Build the interceptor pipeline wrapping the actual execution.
            // Phase 1: Pipeline trägt SleipnirInvocationContext (HttpContext, später InvokeInfo/Activity).
            SleipnirInvocationDelegate pipeline = ctx => ExecuteSingleInvocationSimple(ctx.Request, ctx.HttpContext, ctx.CancellationToken);
            ...
            var invocationContext = new SleipnirInvocationContext
            {
                Request = request,
                HttpContext = context,
        ...
        /// </summary>
        public async Task<SleipnirSubscribeResult> SubscribeAsync(
            SleipnirRequest request, HttpContext? context, CancellationToken ct = default)
        {
            try
            {
                var controllerType = GetControllerType(request.Controller);
                if (controllerType == null)
                    return SleipnirSubscribeResult.Fail(BadRequest($"Controller '{request.Controller}' not found.", HttpStatusCode.NotFound));
                ...
                catch (ForbiddenAccessException) { return SleipnirSubscribeResult.Fail(Forbidden()); }

                // Parameter binden (wie Calls).
                var parameters = BuildParameters(request?.Params, invokeInfo.MethodInfo!.GetParameters(), ct);
                if (parameters.Items == null) return SleipnirSubscribeResult.Fail(parameters.Response!);
                ...
                if (instance == null)
                    return SleipnirSubscribeResult.Fail(InternalServerError($"Controller '{request.Controller}' not registered in DI."));
            ...
            {
                return SleipnirSubscribeResult.Fail(InternalServerError("An internal error occurred while subscribing.", ex));
            }
        }
        ...
        private async Task<SleipnirResponse?> ExecuteSingleInvocationSimple(
        ...
        private async Task<IEnumerable<SleipnirResponse?>> ExecuteInParallel(
            IEnumerable<SleipnirRequest> requests,
            HttpContext? context,
            CancellationToken ct)
        {
        ...
        private async Task<IEnumerable<SleipnirResponse?>> ExecuteSequentially(
            IEnumerable<SleipnirRequest> requests,
            HttpContext? context,
            CancellationToken ct)
        {
            ...
            var orderedResults = new List<SleipnirResponse?>(requestList.Count);
            ...
            foreach (var current in requestList)
            {
                ...
                var effectiveRequest = new SleipnirRequest()
                {
                    Id = current.Id,
        ...
        /// </summary>
        private async Task<IEnumerable<SleipnirResponse?>> ExecuteInDependencyBatches(
            List<SleipnirRequest> requests,
            HttpContext? context,
            CancellationToken ct)
        {
            List<List<SleipnirRequest>> batches;
            try
            {
            ...
            var priorResponses = new ConcurrentDictionary<string, SleipnirResponse?>();
        ...
        /// </summary>
        private static string GraphKey(SleipnirRequest r)
        {
        ...
        /// </summary>
        private async Task<SleipnirResponse?> ExecuteDependentRequestAsync(
            SleipnirRequest request,
            ...
            CancellationToken ct)
        {
            ...
            SleipnirRequest effective = request!;
        ...
        /// <see cref="SleipnirRequest.Params"/>-<see cref="JsonNode"/> (DeepClone, damit das
        /// DTO unverändert bleibt) und reicht den aufgelösten Knoten weiter — kein
        /// String-Roundtrip mehr.
        /// </summary>
        private (JsonNode? ResolvedParams, SleipnirResponse? Error) ResolveParameterValues(
            SleipnirRequest current,
            ConcurrentDictionary<string, SleipnirResponse?>? previousResponses)
        {
        ...
        /// </summary>
        private SleipnirResponse? StrictBindingCheck(SleipnirRequest current, List<AliasReplacement> replacements)
        {
        ...
        /// </summary>
        private SleipnirResponse? ParanoidBindingCheck(SleipnirRequest current, JsonNode parameterNode)
        {
        ...
        /// <summary>
        /// Liest das native <c>data</c>-Feld eines SleipnirParameter-Eintrags und liefert es als
            ...
            if (node is JsonValue jsonValue)
            {
                if (jsonValue.TryGetValue<string>(out var strValue) && strValue.StartsWith("@"))
                {
                    ...
                    if (exposedDependencies.TryGetValue(alias, out var actualValue))
                    {
                        ...
                        // "\"hi\"" (string) oder "{...}" (Objekt). Da SleipnirParameter.Data
                        ...
                        // Alias direkt der Wert eines SleipnirParameter-Objekts ist (Eltern-
        ...
        private readonly struct AuthDecision
        {
            ...
            public readonly SleipnirResponse? Error;
            public AuthDecision(InvokeInfo? info, Type? type, SleipnirResponse? error)
        ...
        /// nicht gefunden, unauthorized) werden mit einem eigenen <c>SleipnirCall</c>-Span getraced,
            ...
            CancellationToken ct)
        {
            ...
            try
            {
                ...
                // nichts — auch nicht, wenn es (wie SleipnirResults.Error(ProblemDetails)) ein
            ...
            catch (Exception ex)
            {
                ...
                SleipnirMetrics.RecordCall(request, errorResponse, durationMs: 0, SleipnirErrorCategory.Internal);
            ...
            SleipnirResponse? Status(SleipnirResponse? response)
            {
                SleipnirTracing.SetCallStatus(activity, response);
                ...
                SleipnirMetrics.RecordCall(request, response, durationMs: 0,
        ...
        /// <summary>
        /// Öffnet einen kurzen <c>SleipnirCall</c>-Span für einen Pre-Execution-Fehler (Auth/Lookup
        ...
        /// </summary>
        private SleipnirResponse? TraceCallError(SleipnirRequest request, SleipnirResponse? response)
        {
            using var activity = SleipnirTracing.StartCall(request);
            SleipnirTracing.SetCallStatus(activity, response);
        ...
        private (object?[]? Items, SleipnirResponse? Response) BuildParameters(
            JsonNode? paramsNode,
            ...
            CancellationToken ct)
        {
            try
            {
                ...
                // statt Deserialize<List<SleipnirParameter>>(string) — kein String-Roundtrip.
                ...
                // Der clientseitige SleipnirCall-Builder vergibt Num ohnehin ohne Kenntnis der
                ...
                foreach (var param in methodParams)
                {
                    ...
                    //    InjectBinaryParameters aus SleipnirRequest.BinaryData gefüllt werden.
        ...
        /// <summary>
        /// Injects binary data from SleipnirRequest.BinaryData into byte[] parameters.
        ...
        private async Task<SleipnirResponse?> ExecuteMethod(
            InvokeInfo invokeInfo,
            Type? controllerType,
            object?[]? parameters,
            CancellationToken ct)
        ...
        private SleipnirResponse ReturnResponse(object? result, Type? instanceType)
        {
            ...
            if (result is SleipnirResponse sleipnirResp)
                return sleipnirResp;
            ...
            if (result is byte[] bytes)
                return new SleipnirResponse { Code = (int)HttpStatusCode.OK, Content = bytes };
        ...
        private async Task CheckAuthorisation(InvokeInfo invokeInfo, HttpContext? context)
        {
            ...
            // Explizite [SleipnirAuthorise] (Method- oder Controller-Level) →
            ...
            if (invokeInfo.AuthoriseAttribute != null)
            {
                ...
                // übernimmt das der SleipnirAuthorizationInterceptor; hier (CheckAuthorisation,
                // gerufen von ResolveAndAuthorizeAsync im Batch-Pre-Pass) nutzen wir den
                // PolicyEvaluator-Delegate, falls gesetzt (von AddSleipnir via SleipnirHub).
        ...
        private SleipnirResponse BadRequest(string message, HttpStatusCode code = HttpStatusCode.BadRequest)
        ...
        private SleipnirResponse Ok(JsonElement? data)
            => new() { Code = SleipnirErrorCodes.Ok, Data = data };

        private SleipnirResponse Ok(byte[]? dataBytes)
            => new() { Code = SleipnirErrorCodes.Ok, DataBytes = dataBytes };

        private SleipnirResponse NoContent()
            => new() { Code = SleipnirErrorCodes.NoContent };

        private SleipnirResponse Unauthorized()
            => CreateError(SleipnirErrorCodes.Unauthorized, "Unauthorized.", SleipnirErrorCategory.Unauthenticated);

        /// <summary>
        /// 403 Forbidden — authentifiziert, aber Rolle/Policy verweigert (Phase 1).
        /// Spiegel von <see cref="SleipnirResults.Forbidden"/>; PermissionDenied-Kategorie.
        /// </summary>
        private SleipnirResponse Forbidden()
            => CreateError(SleipnirErrorCodes.Forbidden, "Forbidden.", SleipnirErrorCategory.PermissionDenied);

        private SleipnirResponse InternalServerError(string message)
            => CreateError(SleipnirErrorCodes.InternalServerError, message, SleipnirErrorCategory.Internal);
        ...
        /// in SleipnirError.Details abgelegt.
        /// </summary>
        private SleipnirResponse InternalServerError(string message, Exception? ex)
        {
            var response = CreateError(SleipnirErrorCodes.InternalServerError, message, SleipnirErrorCategory.Internal);
        ...
        /// </summary>
        private SleipnirResponse CreateError(int code, string? message, SleipnirErrorCategory category = SleipnirErrorCategory.None)
        {
            return new SleipnirResponse
            {
                ...
                Error = new SleipnirError
                {
                    ...
                    Category = category == SleipnirErrorCategory.None ? CategoryFor(code) : category,
                }
            };
        }

        /// <summary>
        /// Leitet die semantische <see cref="SleipnirErrorCategory"/> aus einem numerischen
        ...
        /// </summary>
        private static SleipnirErrorCategory CategoryFor(int code) => code switch
        {
            SleipnirErrorCodes.BadRequest or 422 => SleipnirErrorCategory.InvalidArgument,
            SleipnirErrorCodes.Unauthorized => SleipnirErrorCategory.Unauthenticated,
            SleipnirErrorCodes.Forbidden => SleipnirErrorCategory.PermissionDenied,
            SleipnirErrorCodes.NotFound => SleipnirErrorCategory.NotFound,
            SleipnirErrorCodes.Conflict => SleipnirErrorCategory.Conflict,
            SleipnirErrorCodes.RequestEntityTooLarge or 429 => SleipnirErrorCategory.ResourceExhausted,
            SleipnirErrorCodes.InternalServerError => SleipnirErrorCategory.Internal,
            SleipnirErrorCodes.ServiceUnavailable => SleipnirErrorCategory.Unavailable,
            SleipnirErrorCodes.ClientClosedRequest => SleipnirErrorCategory.Cancelled,
            _ => SleipnirErrorCategory.None,
        };
        ...
        public class InvokeInfo
        {
            ...
            /// </summary>
            public SleipnirAnonymousAttribute? AnonymousAttribute { get; set; }
