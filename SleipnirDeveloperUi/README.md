# Sleipnir Developer UI

The **Sleipnir Developer UI** is the in-browser workbench that ships with every Sleipnir
...
page: there is no hand-written spec, because in Sleipnir **the C# classes are the
...
> The DevUI is served by the Sleipnir host at `/Sleipnir` (configurable). It talks to the
> server over the native REST wire (`POST /api/sleipnir/json`, `POST /api/sleipnir/json/multi`,
> `GET /api/sleipnir/discovery`). No backend rebuild is needed to use it — discovery and
...
browser lands in the Developer UI at **`http://localhost:5001/Sleipnir`**. Six controllers
...
Against any other Sleipnir server: open the DevUI it serves, or point this DevUI at a
...
Scenario: Story01 running (http://localhost:5001/Sleipnir), DARK theme. Full window.
  ...
  - Top bar visible (brand "Sleipnir Developer", API-path pill "api/sleipnir", Settings,
...
The left pane is the live discovery view from `GET /api/sleipnir/discovery`. Every
`[SleipnirController]` and `[SleipnirMethod]` the server registered is listed here, with the
contract types inferred from the method signatures (property schemas, nested types,
`[SleipnirExample]` samples, `[SleipnirDocumentation]` summaries). The filter box narrows by
...
editor (kept in sync). **Run** sends one `POST /api/sleipnir/json`; the response lands in
...
Sleipnir's headline feature is **dependency chaining**: the client declares *what depends
...
"Code kopieren" button visible. The code block shows the generated SleipnirMultiRequest
with six SleipnirRequest entries — Order/GetById with DependencyMapping (customerId/orderId/
...
(the wire payload for `/api/sleipnir/json/multi`). The JSON tab is the fastest way to reproduce
the story: copy it, open an in-tab Batch, and paste. The C# tab mirrors the
`SleipnirMultiRequest`/`SleipnirCall` fluent form from `stories/01-n-plus-one-screen/README.md`.
...
**Ausführen** sends the whole chain as one `POST /api/sleipnir/json/multi`. The server
...
(`SleipnirOptions.AliasBindingMode`).
...
- **API-path pill** — the active target (`api/sleipnir` same-origin, or your custom base/ path).
...
Connection section (Base URL "/ (Same-Origin)…", API path "api/sleipnir", Reset/Apply) and
...
- **Story 01** (`docs/stories/01-the-n-plus-one-screen.md`) — the full N+1 reasoning,
  the REST-vs-Sleipnir comparison, and the one-batch call this walk-through reproduces.
