<script lang="ts">
  ...
  function resetConnection() {
    baseUrlInput = '/';
    apiPathInput = 'api/sleipnir';
    endpointState.set('/', 'api/sleipnir');
    settingsOpen = false;
    discoveryState.fetchDiscovery();
  }
...
<header class="topbar">
  <div class="brand">
    <!-- Sleipnir = French "weft": the cross-threads that hold a fabric together.
         Brand mark: two woven threads (green + blue) instead of three colored dots. -->
    <svg class="brand-mark" width="16" height="16" viewBox="0 0 16 16" fill="none" stroke-width="1.9" stroke-linecap="round" aria-hidden="true" title="Sleipnir — woven threads">
    ...
    <span class="title">Sleipnir Developer</span>
  </div>
  ...
  <div class="actions">
    ...
    <div class="dropdown-wrap">
      ...
      {#if settingsOpen}
        ...
        <div class="auth-panel settings-panel" onclick={(e) => e.stopPropagation()} role="dialog" aria-label="Verbindung & Workspace">
          ...
          <label class="auth-label" for="sleipnir-baseurl">Base URL</label>
          <input
            id="sleipnir-baseurl"
            type="text"
            ...
            spellcheck={false}
          />
          <label class="auth-label" for="sleipnir-apipath">API-Pfad</label>
          <input
            id="sleipnir-apipath"
            type="text"
            bind:value={apiPathInput}
            onkeydown={(e) => e.key === 'Enter' && applyConnection()}
            placeholder="api/sleipnir"
            spellcheck={false}
          />
    ...
    <div class="dropdown-wrap">
      ...
      {#if authOpen}
        ...
        <div class="auth-panel" onclick={(e) => e.stopPropagation()} role="dialog" aria-label="Bearer-Token">
          <label class="auth-label" for="sleipnir-bearer-input">Bearer-Token</label>
          <input
            id="sleipnir-bearer-input"
            type="password"
    ...
    <button class="ghost small" onclick={() => tabState.createCodegenTab()} title="Preview the generated client — live from discovery">
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
