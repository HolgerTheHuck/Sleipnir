<script lang="ts">
  ...
  import { ExecutionMode, type SleipnirRequest, type SleipnirParameter, type ControllerMeta, type MethodMeta, type ParameterMeta } from 'sleipnir-client';
  ...
  let {
    ...
    mode: ExecutionMode;
    onchange?: (requests: SleipnirRequest[], mode: ExecutionMode) => void;
  } = $props();
  ...
  function hydrate(reqs: SleipnirRequest[]): BatchRow[] {
  ...
  // --- row → SleipnirRequest (spiegelt DependencyStep.buildRequest) -------------

  function rowToRequest(row: BatchRow): SleipnirRequest {
    const mm = methodMeta(row);
      ...
      <div class="batch-card">
        <div class="card-header">
          ...
          <input
            ...
            spellcheck={false}
            placeholder="ID"
            title="Korrelations-Id (zugleich SleipnirRequest.id, @alias-Schlüssel)"
          />
          <input
            class="ctrl-input"
            list="batch-ctrls"
