<script lang="ts">
  ...
  let tsSnippet = $derived.by(() => {
    ...
    const args = tab.params
      ...
      .join(',\n');
    return `import { sleipnir } from "./sleipnir-client";

const res = await sleipnir("${ctrl}", "${m}", [
${args}
]);
  ...
  let csSnippet = $derived.by(() => {
    ...
    const args = tab.params
      .map((p) => {
        ...
        return `    new SleipnirCallArg("${p.parameterName}", JsonNode.Parse(${JSON.stringify(json)})!)`;
      })
      .join(',\n');
    return `using var client = new SleipnirClient("/api/sleipnir");
