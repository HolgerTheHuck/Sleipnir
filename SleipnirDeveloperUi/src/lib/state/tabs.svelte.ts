import type { ControllerMeta, MethodMeta, ParameterMeta, SleipnirParameter, TypeRef } from 'sleipnir-client';
...
export interface DepStep {
  /** Schritt-Id — zugleich SleipnirRequest.id. */
  id: string;
  controller: string;
  method: string;
  params: DepParam[];
...
const STORAGE_KEY = 'sleipnir-tabs';
...
function buildDefaultPayload(params: (ParameterMeta & { value: unknown })[]): SleipnirParameter[] {
  return params.map((p) => ({
    ...
    try {
      ...
      if (Array.isArray(arr)) {
        for (const p of tab.params) {
          const match = arr.find((x: SleipnirParameter) => x.parameterName === p.parameterName);
          if (match) {
