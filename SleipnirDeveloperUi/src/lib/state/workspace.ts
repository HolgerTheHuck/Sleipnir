...
// JSON-Datei zusammen, damit man einen fixen Stand (z. B. für einen Sleipnir-
...
// Schutz); er bleibt separat in sleipnir-bearer und greift nach Import weiter.
...
function filename(): string {
  return `sleipnir-workspace-${timestamp()}.json`;
}
  ...
  if (typeof w.showSaveFilePicker === 'function') {
    try {
      const handle = await w.showSaveFilePicker({
        ...
        types: [{ description: 'Sleipnir Workspace', accept: { 'application/json': ['.json'] } }],
      });
...
export function importWorkspaceFromText(json: string): void {
  ...
  const conn = parsed.connection ?? { baseUrl: '/', apiPath: 'api/sleipnir' };
  endpointState.set(conn.baseUrl ?? '/', conn.apiPath ?? 'api/sleipnir');
