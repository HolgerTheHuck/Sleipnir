...
/// <summary>
/// Reiner bidirektionaler Übersetzer JSON-RPC 2.0 ↔ Sleipnir. Keine Transport- und keine
...
/// <c>ISleipnirCore.InvokeDi</c> rufen, Capability-Methoden beantworten).
///
...
/// (Method not found) gemappt; ein <i>Business</i>-404 (<c>SleipnirResults.NotFound</c>)
...
internal static class JsonRpcAdapter
{
    ...
    /// jsonrpc/method/params und übersetzt in einen <see cref="SleipnirRequest"/> (oder
    ...
    public static ParsedRpcItem ParseRequest(JsonElement item)
    {
        ...
        if (method == "sleipnir.discover" || method == "sleipnir.capabilities")
        {
            result.IsValid = true;
        ...
        result.IsValid = true;
        result.Request = new SleipnirRequest
        {
    ...
    /// <summary>
    /// Übersetzt JSON-RPC <c>params</c> in den nativen Sleipnir-<see cref="SleipnirRequest.Params"/>-
    /// <see cref="JsonNode"/> (ein <see cref="JsonArray"/> von <c>{ parameterName, data, num }</c>).
    /// Named (Object) → <see cref="SleipnirParameter.ParameterName"/> = Schlüssel; positional (Array)
    /// → <see cref="SleipnirParameter.Num"/> = Index (der Server fällt bei nicht treffendem Namen
    /// auf den Positional-Index zurück). <see cref="SleipnirParameter.Data"/> ist der native
    ...
    {
        var parameters = new List<SleipnirParameter>();
        if (pEl.HasValue)
        {
            var p = pEl.Value;
            if (p.ValueKind == JsonValueKind.Object)
            {
                foreach (var prop in p.EnumerateObject())
                    parameters.Add(new SleipnirParameter { ParameterName = prop.Name, Data = JsonNode.Parse(prop.Value.GetRawText()) });
            }
            else if (p.ValueKind == JsonValueKind.Array)
            {
                ...
                {
                    parameters.Add(new SleipnirParameter { Num = i, ParameterName = $"param{i}", Data = JsonNode.Parse(el.GetRawText()) });
                    i++;
                }
            }
        }
    ...
    /// <summary>
    /// Mappt einen Sleipnir-Status-Code (plus semantische Kategorie, Phase 1) auf einen
    /// JSON-RPC-Fehlercode. Die Kategorie löst die bisherige String-Präfix-Kopplung ab:
    /// statt <c>errorMessage.StartsWith("Controller '…")</c> wird <see cref="SleipnirErrorCategory.NotFound"/>
    ...
    /// </remarks>
    public static int MapErrorCode(int sleipnirCode, SleipnirCommon.Results.SleipnirErrorCategory category, string? errorMessage)
    {
        ...
        if (sleipnirCode == 404 && errorMessage is not null
        ...
        if (category != SleipnirCommon.Results.SleipnirErrorCategory.None)
        {
            return category switch
            {
                SleipnirCommon.Results.SleipnirErrorCategory.InvalidArgument => -32602,   // Invalid params
                SleipnirCommon.Results.SleipnirErrorCategory.Unauthenticated => -32001,   // Auth (401)
                SleipnirCommon.Results.SleipnirErrorCategory.PermissionDenied => -32001,  // Auth (403)
                SleipnirCommon.Results.SleipnirErrorCategory.NotFound => -32000,          // Business-NotFound (catch-all)
                SleipnirCommon.Results.SleipnirErrorCategory.Conflict => -32000,
                SleipnirCommon.Results.SleipnirErrorCategory.FailedPrecondition => -32602,// Invalid params (Dep-Kette)
                SleipnirCommon.Results.SleipnirErrorCategory.ResourceExhausted => -32000,
                SleipnirCommon.Results.SleipnirErrorCategory.Internal => -32603,          // Internal error
                SleipnirCommon.Results.SleipnirErrorCategory.Unavailable => -32003,       // Server error (overload)
                SleipnirCommon.Results.SleipnirErrorCategory.Cancelled => -32000,         // catch-all (499)
                _ => -32000,
            };
        }

        // Fallback: numerischer Code (v1.0-Verhalten für Responses ohne Category).
        return sleipnirCode switch
        {
    ...
    /// <summary>
    /// Baut ein JSON-RPC-Response-Objekt aus einem <see cref="SleipnirResponse"/> (Erfolg
    ...
    /// </summary>
    public static JsonObject MapResponse(SleipnirResponse sleipnir, JsonElement? id)
    {
        var obj = new JsonObject { ["jsonrpc"] = JsonRpcVersion };
        if (sleipnir.IsSuccess)
        {
            obj["result"] = ResultNode(sleipnir);
        }
        else
        {
            var msg = sleipnir.Error?.Message ?? $"Sleipnir error {sleipnir.Code}.";
            var err = new JsonObject
            {
                ["code"] = MapErrorCode(sleipnir.Code, sleipnir.Error?.Category ?? SleipnirCommon.Results.SleipnirErrorCategory.None, sleipnir.Error?.Message),
                ["message"] = msg,
            };
            // error.data: bevorzugt das strukturierte Sleipnir-Data (z.B. ProblemDetails),
            // sonst die Error.Details als String. Beides null → Feld entfällt.
            var dataNode = ErrorDataNode(sleipnir);
    ...
    /// <summary>
    /// Strukturiertes Ergebnis für <c>sleipnir.capabilities</c> — die statische
    /// Adoptions-Brücke: listet die Sleipnir-Stärken auf, die im JSON-RPC-Compat-Modus
    ...
    {
        ["nativeClient"] = true,                 // SleipnirClient (REST/WS/SignalR) verfügbar
    ...
    private static JsonNode ResultNode(SleipnirResponse sleipnir)
    {
        ...
        if (sleipnir.Data.HasValue)
            return JsonNode.Parse(sleipnir.Data.Value.GetRawText());
        if (sleipnir.Content is not null)
            return Convert.ToBase64String(sleipnir.Content);
        return JsonNull;
    }
    ...
    private static JsonNode? ErrorDataNode(SleipnirResponse sleipnir)
    {
        if (sleipnir.Data.HasValue)
            return JsonNode.Parse(sleipnir.Data.Value.GetRawText());
        if (!string.IsNullOrEmpty(sleipnir.Error?.Details))
            return sleipnir.Error!.Details;
        return null;
    }
