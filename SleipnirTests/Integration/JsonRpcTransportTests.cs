...
/// <c>POST /api/sleipnir/jsonrpc</c>. Echter Kestrel-Host (wie <c>TransportTestFixture</c>),
...
public class JsonRpcTransportTests : IClassFixture<JsonRpcFixture>
{
    ...
    private async Task<(int Status, JsonElement Body)> PostAsync(string body)
    {
        ...
        var resp = await _client.PostAsync("/api/sleipnir/jsonrpc", content);
    ...
    {
        // GetOr404(99) → SleipnirResults.NotFound → Business-404, NICHT Routing → -32000.
    ...
    [Fact]
    public async Task SleipnirDiscover_ReturnsDiscoveryInfo()
    {
        var (_, b) = await PostAsync("""{"jsonrpc":"2.0","method":"sleipnir.discover","id":1}""");
        b.GetProperty("result").ValueKind.Should().Be(JsonValueKind.Object);
        b.GetProperty("id").GetInt32().Should().Be(1);
    }

    [Fact]
    public async Task SleipnirCapabilities_ReturnsManifest()
    {
        var (_, b) = await PostAsync("""{"jsonrpc":"2.0","method":"sleipnir.capabilities","id":2}""");
...
public class JsonRpcFixture : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        app.UseRouting();
        app.UseSleipnir();
        app.MapSleipnirEndpoints("/api/sleipnir", enableJsonRpcCompat: true);
