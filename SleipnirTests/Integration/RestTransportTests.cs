...
/// <summary>
/// Integration tests for the REST transport (POST /api/sleipnir/json, /api/sleipnir/json/multi, GET /api/sleipnir/discovery).
...
public class RestTransportTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    public RestTransportTests(WebApplicationFactory<Program> factory)
    {
        _factory = factory.WithWebHostBuilder(builder =>
        {
            builder.ConfigureServices(services =>
            {
                ...
                var invoker = sp.GetRequiredService<ISleipnirCore>();
    ...
    {
        // Act
        var response = await _client.GetAsync("/api/sleipnir/discovery");
    ...
    public async Task SingleCall_Echo_Returns200()
    {
        ...
        // Act
        var response = await _client.PostAsync("/api/sleipnir/json", CreateJsonContent(request));
    ...
    public async Task MultiCall_Parallel_Returns200()
    {
        ...
        // Act
        var response = await _client.PostAsync("/api/sleipnir/json/multi", CreateJsonContent(multiRequest));
    ...
    public async Task SingleCall_FluentPositional_BindsByIndex()
    {
        ...
        var request = SleipnirCall.Init("TestService", "GetAdresse")
            .With(1, "Hi")
            .ToRequest();
        ...
        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var content = await response.Content.ReadAsStringAsync();
        var sleipnirResp = JsonSerializer.Deserialize<SleipnirResponse>(content, _jsonOpts);
        sleipnirResp!.Code.Should().Be(200);
        sleipnirResp.Data.Value.GetRawText().Should().Contain("\"greet\":\"Hi\"");
        sleipnirResp.Data.Value.GetRawText().Should().Contain("\"id\":1");
    }
    ...
    public async Task SingleCall_FluentNamed_BindsByName()
    {
        ...
        var sleipnirResp = JsonSerializer.Deserialize<SleipnirResponse>(content, _jsonOpts);
        sleipnirResp!.Code.Should().Be(200);
        sleipnirResp.Data.Value.GetRawText().Should().Contain("\"greet\":\"Hola\"");
        sleipnirResp.Data.Value.GetRawText().Should().Contain("\"id\":2");
    }
    ...
    {
        // Arrange
        var step1 = new SleipnirRequest
        {
            Controller = "Customer",
            Method = "AddCustomer",
            Params = JsonSerializer.SerializeToNode(new List<SleipnirParameter>
            {
        ...
        var multi = new SleipnirMultiRequest
        {
            ...
            Mode = ExecutionMode.Parallel,
            Requests = new List<SleipnirRequest> { step1, step2 }
        };

        // Act
        var response = await _client.PostAsync("/api/sleipnir/json/multi", CreateJsonContent(multi));

        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var content = await response.Content.ReadAsStringAsync();
        var responses = JsonSerializer.Deserialize<List<SleipnirResponse>>(content, _jsonOpts);
    ...
    public async Task DependencyChain_StringAlias_PropertyPath_PreservesValue()
    {
        ...
        {
            Mode = ExecutionMode.Parallel,
            Requests = new List<SleipnirRequest> { step1, step2, step3 }
        };
