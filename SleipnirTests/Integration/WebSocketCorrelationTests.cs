...
/// <summary>
/// R3 server-side regression: WebSocket error frames must be real <see cref="SleipnirCommon.Models.SleipnirResponse"/>
/// objects (Code + <see cref="SleipnirCommon.Models.SleipnirError"/> + Id) so a client can surface the
...
/// hid in <c>data</c> where <c>SleipnirException</c> never looks.
/// </summary>
public class WebSocketCorrelationTests : IClassFixture<TransportTestFixture>
{
    ...
    private async Task<JsonElement> SendRawAndReceiveAsync(string json)
    {
        ...
        await ws.ConnectAsync(new Uri(wsBase + "sleipnirws"), CancellationToken.None);
    ...
    public async Task MalformedJson_ReturnsStructuredErrorFrame_NotAnonymousDataEnvelope()
    {
        ...
        // anonymous { code, data } that a client never surfaced as a SleipnirException.
        ...
        // and `data` is null (SleipnirException reads Error.Message, never data).
