...
public class WebSocketTransportTests : IClassFixture<TransportTestFixture>
{
    ...
    public async Task Echo_NonAscii_LargePayload_RoundTrips()
    {
        ...
        var request = SleipnirCall.Init("TestInvoker", "Echo").With(payload).ToRequest();
    ...
    public async Task UploadBlob_DownloadBlob_BinaryRoundTrip()
    {
        ...
        var upload = new SleipnirRequest
        {
            Controller = "TestInvoker",
            Method = "UploadBlob",
            Params = JsonSerializer.SerializeToNode(new[]
            {
                new SleipnirParameter { ParameterName = "filename", Data = JsonSerializer.SerializeToNode("f.bin") }
            }),
            BinaryData = bytes,
        ...
        var download = SleipnirCall.Init("TestInvoker", "DownloadBlob").With("blob").ToRequest();
    ...
    {
        // Cancellation muss als OCE propagieren, nicht als SleipnirException (P0.7).
        var client = _fixture.CreateWsClient();
    ...
    {
        var client = _fixture.CreateWsClient();
        var request = SleipnirCall.Init("Customer.Address.Contact", "Add").With("Alice").ToRequest();
    ...
    {
        // R3 regression: a SleipnirMultiRequest without ids used to hang forever — the client
        ...
        var client = new SleipnirWebSocketClient(_fixture.BaseUrl, callTimeout: TimeSpan.FromSeconds(5));

        // Build two requests and strip their ids — SleipnirCall.Init defaults Id to
        // "Controller.Method"; the regression requires genuinely id-less requests.
        var req1 = SleipnirCall.Init("TestInvoker", "Echo").With("a").ToRequest();
        var req2 = SleipnirCall.Init("TestInvoker", "Echo").With("b").ToRequest();
        req1.Id = null;
        req2.Id = null;
        var multi = new SleipnirMultiRequest
        {
            Mode = ExecutionMode.Parallel,
            Requests = new List<SleipnirRequest> { req1, req2 },
        };
