...
/// <summary>
/// Reconnect tests for <see cref="SleipnirWebSocketClient"/>. Because <see cref="ClientWebSocket"/>
...
/// </summary>
public class SleipnirWebSocketClientReconnectTests
{
    /// <summary>Minimal Kestrel WS server: echoes Sleipnir responses, counts accepts, holds the current socket.</summary>
    ...
    private static SleipnirRequest EchoRequest(string id) => new()
    {
        Controller = "C",
        Method = "M",
    ...
    public async Task Reconnect_NachServerClose_StelltVerbindungWiederHerUndCallGeht()
    {
        ...
        client.State.Should().Be(SleipnirConnectionState.Connected);
        ...
        await WaitForStateAsync(client, SleipnirConnectionState.Reconnecting, timeoutMs: 2000);
    ...
    public async Task Reconnect_BackoffErschoepft_GehtInDisconnected()
    {
        ...
        await server.CloseCurrentAsync();
        await WaitForStateAsync(client, SleipnirConnectionState.Disconnected, timeoutMs: 3000);
        client.State.Should().Be(SleipnirConnectionState.Disconnected);
    ...
    [Fact]
    public async Task InFlightCall_BeiDrop_WirftSleipnirException()
    {
