...
/// spezifizierte Laufzeitverhalten end-to-end über den SleipnirInvoker: ein Provider
...
public class AliasBindingTests
{
    ...
    /// <summary>Baut einen SleipnirRequest mit Id, benannten Parametern (Data als JsonNode)
    ...
    private static SleipnirRequest DtoProvider(string id, int dtoId, string dtoName,
    ...
    private static SleipnirRequest IdOnlyProvider(string id, int dtoId, string alias, string jsonPath) =>
        Req(id, "DepChain", "MakeIdOnly",
            mapping: new() { [alias] = jsonPath },
            ("id", dtoId.ToString()));

    private static SleipnirRequest StringIdProvider(string id, string dtoId, string alias, string jsonPath) =>
