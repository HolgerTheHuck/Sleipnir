...
///
/// <c>[SleipnirAuthorise(Policy=...)]</c> was evaluated only by the
/// <c>SleipnirAuthorizationInterceptor</c> on the single-call path; the batch path
...
/// <c>AddSleipnir</c> when <c>IAuthorizationService</c> is available, and <c>CheckAuthorisation</c>
/// now evaluates it in the serial pre-pass. <c>PolicyEvaluator</c> appeared nowhere in
/// <c>SleipnirTests</c> before this — a security hotfix without a regression test.
/// </summary>
...
public class BatchPolicyAuthTests
{
    ...
    public BatchPolicyAuthTests()
    {
        ...
        // Mirror the AddSleipnir wiring: a PolicyEvaluator that grants "allowed" and denies
    ...
    private static SleipnirRequest Req(string id, string method,
