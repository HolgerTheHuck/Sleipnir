...
/// <summary>
/// Structural TypeRef edge-case coverage for <see cref="SleipnirDiscoveryService"/>. The base
/// <see cref="SleipnirDiscoveryServiceTests"/> exercise the common kinds (scalar, array, map,
...
/// <c>[SleipnirExample]</c> example population, a self-referential type (cycle-safe placeholder),
/// bare <c>Task</c> → <c>void</c>, and the deterministic <see cref="DiscoverySerialization"/>
/// wire contract (camelCase + null omission).
/// </summary>
public class SleipnirDiscoveryTypeRefTests
{
    private static SleipnirInvoker CreateInvoker<T>() where T : class
    {
    ...
    [Fact]
    public void SleipnirExample_PopulatesTypeExample()
    {
        var d = CreateInvoker<DiscoveryEdgeCasesController>().GetDiscoveryInfo();
    ...
    public void GetDiscoveryInfo_CachesUntilInvalidated()
    {
        ...
        // A re-registration invalidates the cache (SleipnirInvoker.Register wires InvalidateCache).
