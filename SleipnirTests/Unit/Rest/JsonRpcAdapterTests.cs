...
namespace SleipnirTests.Unit.Rest;
...
/// (Body, Batch, Notifications, ISleipnirCore-Aufruf) ist in <c>JsonRpcTransportTests</c>
/// (Integration) geprüft.
/// </summary>
public class JsonRpcAdapterTests
{
    ...
    public void ParseRequest_NamedParams_BuildsStringDataWithParameterNames()
    {
        ...
        // Params ist eine List<SleipnirParameter> als JsonArray; Data ist das rohe Fragment (JsonNode).
        var paramsList = JsonSerializer.Deserialize<List<SleipnirParameter>>(parsed.Request.Params!);
    ...
    {
        var discover = JsonRpcAdapter.ParseRequest(Item("""{"jsonrpc":"2.0","method":"sleipnir.discover","id":1}"""));
        discover.IsValid.Should().BeTrue();
        discover.Capability.Should().Be("sleipnir.discover");
        discover.Request.Should().BeNull();

        var caps = JsonRpcAdapter.ParseRequest(Item("""{"jsonrpc":"2.0","method":"sleipnir.capabilities","id":2}"""));
        caps.Capability.Should().Be("sleipnir.capabilities");
    ...
    {
        JsonRpcAdapter.MapErrorCode(404, SleipnirErrorCategory.None, "Controller 'Ghost' not found.").Should().Be(-32601);
        JsonRpcAdapter.MapErrorCode(404, SleipnirErrorCategory.None, "Method 'X' not found on controller 'Y'.").Should().Be(-32601);
    }
    ...
    public void MapErrorCode_Business404_IsServerError()
    {
        ...
        JsonRpcAdapter.MapErrorCode(404, SleipnirErrorCategory.None, "Customer '42' not found.").Should().Be(-32000);
        // Mit Category=NotFound → -32000 (Business-NotFound, catch-all).
        JsonRpcAdapter.MapErrorCode(404, SleipnirErrorCategory.NotFound, "Customer '42' not found.").Should().Be(-32000);
    }
    ...
    public void MapErrorCode_TrimeTypeToRpc(int sleipnir, int expected)
        => JsonRpcAdapter.MapErrorCode(sleipnir, SleipnirErrorCategory.None, "x").Should().Be(expected);

    [Theory]
    // Phase 1: Category-basierte Map (präziser als numerischer Code allein).
    [InlineData(SleipnirErrorCategory.InvalidArgument, -32602)]
    [InlineData(SleipnirErrorCategory.Unauthenticated, -32001)]
    [InlineData(SleipnirErrorCategory.PermissionDenied, -32001)]
    [InlineData(SleipnirErrorCategory.NotFound, -32000)]
    [InlineData(SleipnirErrorCategory.Conflict, -32000)]
    [InlineData(SleipnirErrorCategory.FailedPrecondition, -32602)]
    [InlineData(SleipnirErrorCategory.ResourceExhausted, -32000)]
    [InlineData(SleipnirErrorCategory.Internal, -32603)]
    [InlineData(SleipnirErrorCategory.Unavailable, -32003)]
    [InlineData(SleipnirErrorCategory.Cancelled, -32000)]
    public void MapErrorCode_CategoryBasedMap(SleipnirErrorCategory category, int expected)
    ...
    {
        var sleipnir = new SleipnirResponse
        {
            Code = 200,
        ...
        var obj = JsonRpcAdapter.MapResponse(sleipnir, id);
    ...
    {
        var sleipnir = new SleipnirResponse { Code = 200, DataBytes = Encoding.UTF8.GetBytes("\"hi\"") };
        var obj = JsonRpcAdapter.MapResponse(sleipnir, IdElement("\"req-1\""));
    ...
    {
        var sleipnir = new SleipnirResponse { Code = 204 };
        var obj = JsonRpcAdapter.MapResponse(sleipnir, IdElement("1"));
    ...
    public void MapResponse_BinaryContent_ResultIsBase64String()
    {
        ...
        var sleipnir = new SleipnirResponse { Code = 200, Content = bytes };
        var obj = JsonRpcAdapter.MapResponse(sleipnir, IdElement("1"));
    ...
    public void MapResponse_Error_UsesMappedCodeAndMessageAndData()
    {
        var sleipnir = new SleipnirResponse
        {
            ...
            Error = new SleipnirError { Code = 422, Message = "Invalid input." },
        };
        var obj = JsonRpcAdapter.MapResponse(sleipnir, IdElement("9"));
    ...
    public void MapResponse_ErrorWithDetailsString_DataIsDetails()
    {
        ...
        {
            Code = 400,
            Error = new SleipnirError { Code = 400, Message = "bad", Details = "ParameterName=value" },
        };
        var obj = JsonRpcAdapter.MapResponse(sleipnir, IdElement("1"));
    ...
    [Fact]
    public void CapabilitiesManifest_ListsSleipnirStrengths()
    {
