// Server-side Trame contract export + drift-check.
//
...
// [TrameController] types, build a TrameInvoker, call GetDiscoveryInfo, serialize with the same
// DiscoverySerialization.Options the REST endpoint uses) and either:
//   - drift-checks it against a committed contract.trame.json and fails (exit 1) on mismatch, or
//   - regenerates the committed file when --regen is passed (used by TRAME_REGEN_GOLDEN=1).
//
// It runs in its own process (invoked by the Trame.Server.Codegen MSBuild target), so loading the
// server assembly and its TrameCore/TrameCommon dependencies is isolated from the MSBuild process
// — no version collisions, no locked files. The tool and the server share the same TrameCore build
// in this repo, so the server's [TrameController] references bind to the already-loaded TrameCore;
...
namespace Trame.Server.Codegen;
...
internal static class Program
{
    ...
    private static int Main(string[] args)
    {
        ...
        catch (Exception ex)
        {
            ...
            Console.Error.WriteLine($"[trame-export] error: {ex}");
            return ExitError;
        }
    }
    ...
    private static int Run(string[] args)
    {
        ...
        if (!File.Exists(opts.ContractPath))
        {
            ...
            Console.WriteLine($"[trame-export] created committed contract at '{opts.ContractPath}'.");
            return ExitOk;
        }
        ...
        {
            Console.WriteLine($"[trame-export] drift-check passed: '{opts.ContractPath}' matches runtime discovery.");
            return ExitOk;
        }
        ...
        {
            File.WriteAllText(opts.ContractPath, regeneratedJson);
            Console.WriteLine($"[trame-export] regenerated committed contract at '{opts.ContractPath}'.");
            return ExitOk;
        }
    ...
    /// <summary>Load the server assembly, reflect all [TrameController] types, build a TrameInvoker
    ...
    private static string RegenerateContract(string serverAssemblyPath)
    {
        ...
        // (the shared TrameCore/TrameCommon are already loaded from the tool's own build; this
        ...
        var invoker = new TrameInvoker(
            ...
            NullLogger<TrameInvoker>.Instance);
    ...
    private static List<Type> DiscoverControllers(Assembly serverAsm, string serverDir)
    {
        ...
        // [TrameController] types. Scoped to the server dir (NOT AppDomain-wide) so that an MSBuild
        // run exporting multiple servers never cross-pollinates controllers — the collision lesson
        // from DiscoveryContractTests (two [TrameController("Customer")] in one AppDomain).
        foreach (var asm in LoadAssembliesIn(serverAsm, serverDir))
        {
            foreach (var t in SafeGetTypes(asm))
            {
                if (!t.IsClass || t.IsAbstract) continue;
                if (t.GetCustomAttribute<TrameControllerAttribute>() is null) continue;
                // AutoDiscover=false controllers are excluded from the bulk auto-discovery scans in
                // AddTrame/UseTrame/FromAssemblies; a server that exposes them does so via an
                // explicit Register<T>(). The export cannot know which opt-outs the server actually
                // registered, so it registers every [TrameController] type it can see — matching a
    ...
    private static void ReportDrift(JsonNode regenNode, JsonNode committedNode, Options opts)
    {
        ...
        Console.WriteLine($"[trame-export] contract drift detected for '{opts.ContractPath}'.");
        Console.WriteLine("The committed contract.trame.json does not match the server's runtime discovery.");
        Console.WriteLine("Either update the server's [TrameController]/[TrameMethod] declarations to match the");
        Console.WriteLine("committed contract, or regenerate the contract if the change is intentional:");
        Console.WriteLine($"    TRAME_REGEN_GOLDEN=1 dotnet build  (regenerates {opts.ContractPath})");
        Console.WriteLine();
    ...
    private static Options? ParseArgs(string[] args)
    {
        ...
        {
            Console.Error.WriteLine("[trame-export] missing required --assembly <path> and/or --contract <path>.");
            PrintHelp();
            return null;
        }
    ...
    {
        Console.Error.WriteLine("Trame contract export + drift-check.");
        Console.Error.WriteLine("Usage: Trame.Server.Codegen --assembly <server.dll> --contract <contract.trame.json> [--regen]");
        Console.Error.WriteLine("  --assembly  Path to the built server assembly (its output dir supplies deps).");
        Console.Error.WriteLine("  --contract  Path to the committed contract.trame.json to drift-check / regenerate.");
