...
/// and a TestServer for REST/Trame-REST benchmarks.
/// </summary>
public class BenchmarkFixture
{
    ...
    public TrameRestJsonClient TrameRestClient { get; private set; } = null!;
    public TrameWebSocketClient TrameWebSocketClient { get; private set; } = null!;
    public TrameSignalrClient TrameSignalrClient { get; private set; } = null!;
    ...
    public void Initialize()
    {
        ...
        _realServer = Microsoft.AspNetCore.WebHost.CreateDefaultBuilder()
            ...
            .ConfigureServices(services =>
            {
                services.AddControllers()
                    .AddApplicationPart(typeof(Trame.Api.CustomerHandler).Assembly);
                ...
                services.AddTrame(new TrameHub.Extensions.TrameOptions
                {
            ...
            .Configure(app =>
            {
                ...
                app.UseTrame();
                app.UseWebSockets();
                app.UseTrameWebSocket("/tramews");
                app.UseEndpoints(endpoints =>
                {
                    endpoints.MapControllers();
                    endpoints.MapTrameEndpoints();
                    endpoints.MapHub<TrameHub.Hub.TrameHub>("/tramehub");
        ...
        // Use the real server's HTTP client for REST and Trame REST
        ...
        TrameRestClient = new TrameRestJsonClient(ServerUrl, RestClient);
        ...
        TrameWebSocketClient = new TrameWebSocketClient(ServerUrl, callTimeout: TimeSpan.FromSeconds(15));
        TrameWebSocketClient.ConnectAsync().GetAwaiter().GetResult();

        // Connect SignalR client
        TrameSignalrClient = new TrameSignalrClient(ServerUrl);
    ...
    public async Task<List<Trame.Model.Customer>?> RestGetAllCustomers()
    {
        var response = await RestClient.GetAsync("api/customer/all");
        response.EnsureSuccessStatusCode();
        return JsonSerializer.Deserialize<List<Trame.Model.Customer>>(await response.Content.ReadAsStringAsync(), JsonOpts);
    }

    public async Task<Trame.Model.Customer?> RestGetCustomerById(int id)
    {
        var response = await RestClient.GetAsync($"api/customer/{id}");
        response.EnsureSuccessStatusCode();
        return JsonSerializer.Deserialize<Trame.Model.Customer>(await response.Content.ReadAsStringAsync(), JsonOpts);
    }

    public async Task<List<Trame.Model.Order>?> RestGetOrders(int orderId)
    {
        var response = await RestClient.GetAsync($"api/customer/{orderId}/orders");
        response.EnsureSuccessStatusCode();
        return JsonSerializer.Deserialize<List<Trame.Model.Order>>(await response.Content.ReadAsStringAsync(), JsonOpts);
    }
    ...
    public void Dispose()
    {
        TrameWebSocketClient?.DisposeAsync().AsTask().GetAwaiter().GetResult();
        TrameRestClient?.Dispose();
        RestClient?.Dispose();
        GrpcChannel?.Dispose();
        _realServer?.Dispose();
    }
