...
{
    public class TrameMultiCallResponse
    {
        ...
        public List<TrameResponse?>? Responses { get; set; }

        public static async Task<TrameMultiCallResponse> Call(ITrameClient client, TrameMultiRequest request)
        {
            var response = await client.Call(request);
            return new TrameMultiCallResponse()
            {
        ...
        public T? Get<T>(string name)
        {
            ...
            // Fallback über Data wie in TrameClientBase.Call<T>.
        ...
        /// </summary>
        public TrameError? GetError(string name)
        {
            var theResult = Responses?.FirstOrDefault(s => s?.Id == name);
            if (theResult is null or { IsSuccess: true })
                return null;

            return theResult.Error ?? TrameError.FromResponse(theResult);
        }
    }
}