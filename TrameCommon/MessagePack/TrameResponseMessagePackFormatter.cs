// ACHTUNG: Diese Datei wird NICHT in TrameCommon kompiliert (TrameCommon referenziert
// nur MessagePack.Annotations, nicht die volle MessagePack-Assembly). Sie wird per
// <Compile Include> in TrameHub.csproj (MessagePack 2.5.187 = Server) UND
// TrameClient.csproj (MessagePack 3.1.3 = Client) gelinkt — derselbe Source kompiliert
...
namespace TrameCommon.MessagePack;

/// <summary>
/// MessagePack-Formatter für <see cref="TrameResponse"/> (SignalR-Kanal, Server +
...
/// </summary>
/// <remarks>
/// Single-Pass-Optimierung: ist <see cref="TrameResponse.DataBytes"/> gesetzt (Bulk-
...
/// (rohe UTF-8-Bytes → MP-Tokens, ein Pass) — der lazy <see cref="TrameResponse.Data"/>-
...
/// </remarks>
public sealed class TrameResponseMessagePackFormatter : IMessagePackFormatter<TrameResponse>
{
    public static readonly TrameResponseMessagePackFormatter Instance = new();

    public void Serialize(ref MessagePackWriter writer, TrameResponse value, MessagePackSerializerOptions options)
    {
        if (value is null)
        {
            writer.WriteNil();
        ...
        // [5] Error: TrameError? — Sub-Formatter über den aktiven Resolver.
        if (value.Error is { } error)
        {
            options.Resolver.GetFormatterWithVerify<TrameError>()
    ...
    public TrameResponse Deserialize(ref MessagePackReader reader, MessagePackSerializerOptions options)
    {
