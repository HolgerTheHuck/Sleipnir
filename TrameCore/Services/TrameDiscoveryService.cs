...
namespace TrameCore.Services
{
    ...
    /// on <see cref="InvalidateCache"/> (wired by TrameInvoker.Register).
    /// </summary>
    public class TrameDiscoveryService
    {
        private readonly ConcurrentDictionary<string, Type> _routeHandlers;
        private DiscoveryInfo? _cachedDiscovery;
        private readonly object _cacheLock = new();

        public TrameDiscoveryService(ConcurrentDictionary<string, Type> routeHandlers)
        {
        ...
        private DiscoveryInfo BuildDiscoveryInfo()
        {
            ...
            // (BCL, Trame framework envelopes, third-party) stay opaque unless [TrameDataContract]
            ...
            foreach (var kvp in _routeHandlers)
            {
                ...
                // Collect every method decorated with [TrameMethod].
                var methods = controllerType.GetMethods()
                    .Where(m => m.GetCustomAttributes(typeof(TrameMethodAttribute), false).Any());

                foreach (var method in methods)
                {
                    var methodAttr = method.GetCustomAttribute<TrameMethodAttribute>();
                    ...
                    var methodDoc = method.GetCustomAttribute<TrameDocumentationAttribute>()?.Summary;
                    ...
                    var paramDoc = method.GetCustomAttribute<TrameDocumentationAttribute>()?.Summary;
        ...
        {
            // Example: [TrameExample] JSON, else a parameterless-ctor default instance, else null.
            var exampleAttr = type.GetCustomAttribute<TrameExampleAttribute>();
            if (exampleAttr != null)
            {
                try
                {
        ...
        ///   2. [TrameDataContract(Exclude = true)] -> force-opaque.
        ///   3. [TrameDataContract] (bare) -> force-expand.
        ///   4. assembly in the contract-assembly set -> expand.
        ///   5. otherwise (foreign/BCL/Trame envelope) -> opaque.
        /// </summary>
        ...
        private static TrameDataContractAttribute? GetDataContractAttribute(Type type)
            => type.GetCustomAttributes(typeof(TrameDataContractAttribute), false)
                   .OfType<TrameDataContractAttribute>()
                   .FirstOrDefault();
