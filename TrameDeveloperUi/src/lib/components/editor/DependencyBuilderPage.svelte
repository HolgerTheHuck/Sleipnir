<script lang="ts">
  ...
  // baut aus DepStep[] → TrameRequest[] (Serial-Modus), zeigt den Graphen live,
  ...
  import { ExecutionMode, type TrameRequest, type TrameParameter } from 'trame-client';
  ...
  // --- TrameRequest-Transform (Kernlogik) -------------------------------------
  ...
  /** Baut einen TrameRequest aus einem DepStep (mit echten Parameter-Namen). */
  function buildRequest(step: DepStep): TrameRequest {
    const arr: TrameParameter[] = step.params.map((p, i) => ({
  ...
  function toWireRequest(step: DepStep): TrameRequest {
  ...
  function generateTs(): string {
    ...
    const fluentHints = steps
      ...
      .map((s) => {
        ...
        return `// TrameCall.init("${s.controller}", "${s.method}").named("${s.id}")${litStr ? `.with({ ${litStr} })` : ''}${exposes}.toRequest()`;
      })
    ...
    return `${fluentBlock}import { ExecutionMode, type TrameMultiRequest } from "trame-client";

// Batch mit @alias-Abhängigkeitskettung — Serial-Modus zwingend.
const batch: TrameMultiRequest = {
  ...
  function generateCs(): string {
    ...
    const fluentHints = steps
      ...
      .map((s) => {
        ...
        return `// TrameCall.Init("${s.controller}", "${s.method}").Named("${s.id}")${litStr}${exposes}.ToRequest()`;
      });
    const fluentBlock = fluentHints.length > 0
      ? '\n// Fluent (nur wenn @alias der einzige Parameter ist):\n' + fluentHints.map((h) => `// ${h}`).join('\n') + '\n'
      : '';

    return `using TrameClient.Trame;
