// Automated drift-detection gate for the server-side export tool (Trame.Server.Codegen).
//
// Slice 3 wired the export tool into Story01's build (AfterBuild target → drift-check on every
// `dotnet build Trame.sln`), but that only secures the *happy path* (committed contract matches
...
//
// The tool runs in its own process (dotnet Trame.Server.Codegen.dll ...), loading the built
// Story01 assembly exactly as the MSBuild target does. Requires `dotnet build Trame.sln` first
// (Story01 and the tool are not ProjectReferences of TrameTests — same constraint as
...
public class ServerCodegenDriftTests
{
    ...
    private static string ResolveToolDll()
    {
        ...
        foreach (var cfg in ConfigOrder())
        {
            var dll = Path.Combine(repo.FullName, "Trame.Server.Codegen", "bin", cfg, "net8.0", "Trame.Server.Codegen.dll");
            if (File.Exists(dll)) return dll;
        }
        throw new FileNotFoundException(
            "Trame.Server.Codegen.dll not built. Run `dotnet build Trame.sln` first.");
    }

    private static string ResolveCommittedContract()
    {
        var repo = ResolveRepoRoot();
        return Path.Combine(repo.FullName, "stories", "01-n-plus-one-screen", "contract.trame.json");
    }
    ...
    public void HappyPath_CommittedContract_MatchesRuntime_ExitZero()
    {
        ...
        File.Exists(committed).Should().BeTrue("the Story01 contract must be committed first (regenerate via TRAME_REGEN_GOLDEN=1 dotnet build)");
        var committedJson = File.ReadAllText(committed);

        var tmp = Path.Combine(Path.GetTempPath(), "trame-drift-test-happy.json");
    ...
    public void DriftPath_TamperedContract_IsDetected_ExitOne_FileUntouched()
    {
        ...
        var tmp = Path.Combine(Path.GetTempPath(), "trame-drift-test-tampered.json");
    ...
    public void RegenPath_TamperedContract_IsRepaired_ExitZero_FileMatchesRuntime()
    {
        ...
        var tmp = Path.Combine(Path.GetTempPath(), "trame-drift-test-regen.json");
        File.WriteAllText(tmp, tampered);
