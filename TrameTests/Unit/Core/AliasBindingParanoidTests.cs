...
public class AliasBindingParanoidTests
{
    ...
    private async Task<TrameResponse> RunSingle(TrameRequest req)
    {
    ...
    private async Task<TrameResponse> RunBatch(params TrameRequest[] batch)
    {
        var responses = (await _invoker.InvokeDi(batch, null, ExecutionMode.Serial)).ToList();
        return responses.Single(r => r?.Id == batch[^1].Id)!;
    }

    private static TrameInvoker NewInvoker(AliasBindingMode mode)
    {
        var services = new ServiceCollection();
        services.AddLogging();
        services.AddTransient<DependencyChainController>();
