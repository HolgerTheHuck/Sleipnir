...
public class AliasBindingStrictTests
{
    ...
    private static TrameRequest DtoProvider(string id, int dtoId, string dtoName, string alias, string path) =>
        Req(id, "DepChain", "MakeDto", new() { [alias] = path },
            ("id", dtoId.ToString()), ("name", JsonSerializer.Serialize(dtoName)));

    private static TrameRequest IdOnlyProvider(string id, int dtoId, string alias, string path) =>
