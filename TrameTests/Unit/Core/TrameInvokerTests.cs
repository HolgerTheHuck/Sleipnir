...
/// <summary>
/// Unit tests for TrameInvoker – the core RPC invocation engine.
/// </summary>
public class TrameInvokerTests
{
    private readonly IServiceProvider _serviceProvider;
    private readonly TrameInvoker _invoker;

    public TrameInvokerTests()
    {
    ...
    public void Register_NonControllerType_DoesNotRegister()
    {
        ...
        // Act: Register a type without [TrameController]
    ...
    // Gleichnamige Trame-Methoden auf demselben Controller sind verboten — Trame
    ...
    // Zwei verschiedene Controllertypen mit demselben Trame-Controller-Namen
    ...
    // für Auto-Discovery (UseTrame) plus explizite Registrierung im selben Host.
    [Fact]
    ...
    public async Task InvokeDi_SyncEcho_ReturnsEchoedString()
    {
        ...
        response.Data.Value.GetRawText().Should().Contain("Hello Trame");
    }
    ...
    // Weg A: Eine Controller-Methode, die ein TrameResponse-Objekt zurückgibt
    // (TrameResults.Error/Ok), muss der Invoker UNVERÄNDERT durchreichen — Code,
    // Data und strukturiertes Error bleiben erhalten (kein Re-Wrap als 200).
    [Fact]
    public async Task InvokeDi_ControllerReturnsTrameResults_Error_IsPassedThroughVerbatim()
    {
    ...
    [Fact]
    public async Task InvokeDi_ControllerReturnsTrameResults_Ok_IsPassedThroughVerbatim()
    {
    ...
    [Fact]
    public async Task InvokeDi_ControllerReturnsTrameResults_BadRequest_CarriesDetails()
    {
    ...
    public async Task InvokeDi_PositionalParams_BindByIndex()
    {
        ...
        var paramList = new List<TrameParameter>
        {
    ...
    {
        // Arrange: [TrameAuthorise] on Secured; no HttpContext -> auth fails.
    ...
    /// TrameParameter[] serialisiert; jsonValue ist jeweils die JSON-Form des Werts
    /// (z. B. "7", "true", "\"hi\"", "{\"Id\":7}").
    /// </summary>
    ...
    //
    // Diese Controller sind absichtlich invalid (gleichnamige Trame-Methoden bzw.
    // gleichnamige Controller). Sie tragen AutoDiscover = false, damit der
    // Auto-Discovery-Skan in UseTrame/AddTrame/FromAssemblies sie überspringt —
    ...
    /// <summary>
    /// Controller mit zwei gleichnamigen Trame-Methoden (unterschiedliche C#-Signaturen).
    /// Trame resolved keine Überladungen anhand der Parameter — der Doppelname ist
    /// ein Bug, keine Feature, und muss bei der Registrierung werfen.
    /// </summary>
    [TrameController("DuplicateNames", AutoDiscover = false)]
    ...
    /// <summary>Erster von zwei Controllern mit gleichem Trame-Namen.</summary>
    [TrameController("Clash", AutoDiscover = false)]
    ...
    /// <summary>Zweiter Controller, der denselben Trame-Namen wie ClashA belegt.</summary>
    [TrameController("Clash", AutoDiscover = false)]
