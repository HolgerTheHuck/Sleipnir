...
/// (Default-Cap aus <c>TrameOptions.EventBufferCapacity</c>,Fallback 100). Ist der
/// Channel voll, wird das älteste Element gedroppt und <c>trame.event.dropped</c>
...
/// </para>
/// </remarks>
internal sealed class TrameSubscriptionManager : IAsyncDisposable
{
    ...
    public TrameSubscriptionManager(WebSocket webSocket, ITrameCore trameCore, ILogger? logger, int bufferCapacity = 100)
    {
    ...
    public async Task<TrameResponse?> HandleSubscribeAsync(TrameRequest request, HttpContext? context, CancellationToken ct)
    {
        var result = await _trameCore.SubscribeAsync(request, context, ct);
    ...
    private sealed class EventObserver<T> : IObserver<T>
    {
        ...
        public void OnNext(T value)
        {
            ...
            var frame = JsonSerializer.Serialize(new
            {
                ...
                eventId,
                data = value,
            }, TrameJsonOptions.Default);
            if (!_state.Buffer.Writer.TryWrite(frame))
            {
                TrameMetrics.EventDropped(_subscriptionId);
        ...
        {
            var frame = JsonSerializer.Serialize(new { type = "complete", subscriptionId = _subscriptionId }, TrameJsonOptions.Default);
        ...
        {
            var frame = JsonSerializer.Serialize(new { type = "error", subscriptionId = _subscriptionId, message = error.Message }, TrameJsonOptions.Default);
            _state.Buffer.Writer.TryWrite(frame);
            _state.Buffer.Writer.TryComplete();
        }
    }
