...
import { TrameRestClient } from "../../src/rest.js";
...
describe("TrameRestClient", () => {
  it("call() POSTet nach /api/trame/json und liefert geparste Response (camelCase Body)", async () => {
    let captured: any;
    const fetch = vi.fn(async (url: string, init: any) => {
      captured = { url, init };
      return okResponse({ code: 200, data: "ok", id: "C.M", isSuccess: true });
    }) as unknown as FetchImpl;
    const c = new TrameRestClient("http://x", { fetch });
    const r = await c.call("C", "M", { id: 1 });
    expect(r.code).toBe(200);
    expect(r.data).toBe("ok");
    expect(captured.url).toBe("http://x/api/trame/json");
  ...
  it("setzt Authorization-Header bei bearer", async () => {
    ...
    const c = new TrameRestClient("http://x", { fetch, bearer: "tok" });
    await c.call("C", "M", {});
    expect(captured.headers["Authorization"]).toBe("Bearer tok");
    expect(captured.headers["Content-Type"]).toBe("application/json");
  });

  it("discover() GETet /api/trame/discovery", async () => {
    let captured: any;
    const fetch = vi.fn(async (url: string, init: any) => {
      captured = { url, init };
    ...
    expect(captured.url).toBe("http://x/api/trame/discovery");
  ...
  it("callBatch() POSTet nach /json/multi, setzt leere Ids und liefert Array", async () => {
    ...
    expect(captured.url).toBe("http://x/api/trame/json/multi");
    const body = JSON.parse(captured.init.body);
  ...
  it("logischer Nicht-2xx-Code (in 200-Body): call liefert Response, callJson wirft TrameError", async () => {
    ...
    await expect(c.callJson("C", "M", {})).rejects.toBeInstanceOf(TrameError);
  });

  it("non-2xx HTTP -> synthetische Response; callBinary wirft TrameError", async () => {
  ...
  it("Abbruch (aborted signal) -> CancelledError, nicht TrameError", async () => {
  ...
  it("Timeout -> CancelledError mit timedOut=true", async () => {
    ...
    const c = new TrameRestClient("http://x", { fetch, callTimeout: 25 });
