...
Erhoben via Subagent gegen `TrameCore/Services/TrameInvoker.cs` und Drumherum. Die drei
Speziallocken:

- **Autorisierung** — `CheckAuthorisation` (`TrameInvoker.cs` Z. 1818–1845) +
  `[TrameAuthorise].OnAuthorization` (`TrameCore/Attributes/TrameAuthoriseAttribute.cs` Z. 56–84,
...
- **Telemetrie** — `TrameCore/Tracing/TrameTracing.cs` (`ActivitySource "Trame"`), aufgerufen an
...
- **Fehler** — zwei parallele Fabriken ohne gemeinsame Konstanten: `TrameResults`
  (`TrameCommon/Results/TrameResults.cs`, Controller-Seite) und private Fabriken im Invoker
  (`TrameInvoker.cs` Z. 1849–1898). Magic Numbers. `TrameError.RequestId` wird im Framework-Pfad
...
`ITrameInterceptor` (`TrameCore/Services/ITrameInterceptor.cs`) existiert, ist aber:
nur im Single-Call-Pfad angebunden (`InvokeDi(TrameRequest)` Z. 230–237); ohne `HttpContext` im
Delegate; mit ungenutztem `TrameInvocationContext`; ohne `TrameOptions.Interceptors`-Collection;
von Batch-Pfaden komplett umgangen. `TrameLoggingInterceptor` ist der einzige Built-in.
...
public sealed class TrameInvocationContext
{
    ...
    public TrameResponse? Response { get; set; }  // nach Execution, für Tracing/Logging
    public Activity? Activity { get; set; }       // der TrameCall-Span
    public CancellationToken CancellationToken { get; init; }
}
```

Ersetzt das ungenutzte `TrameInvocationContext` und reicht `HttpContext` durch — die
Schlüssellücke heute.

### Interceptor-Typen

- `ITrameInterceptor` (bestehend, erweitert) — pro Request-Invocation, bekommt den Context.
- `ITrameBatchInterceptor` (neu) — pro Batch, läuft *um* die Batch-Ausführung herum (Metrics,
...
- **Tracing umschließt Method-Invocation** — der `TrameCall`-Span bleibt, wie er ist; der
...
**1. `TrameAuthorizationInterceptor` (Punkt 1 — Policies)**
- Übernimmt `CheckAuthorisation`-Logik + `[TrameAuthorise].OnAuthorization`.
- Erweitert `[TrameAuthorise]` um `Policy` (an ASP.NET Core `IAuthorizationService`).
- `[TrameAnonymous]` wird zur Skip-Annotation (Interceptor prüft `InvokeInfo.AnonymousAttribute`
...
**2. `TrameTelemetryInterceptor` (Punkt 4 — OTel Metrics/Logging)**
- Übernimmt die acht `TrameTracing.*`-Aufrufe — eine Stelle.
- Erweitert um Metrics (`Meter "Trame"`: `trame.call.duration` Histogram,
  `trame.call.count` Counter, `trame.batch.fan_out`, `trame.error.rate`).
- Logging-Conventions: strukturierte Felder passend zu OTel-RPC-Semantic-Conventions.
- `ITrameBatchInterceptor`-Variante für `trame.batch.*`-Spans/Metrics.

**3. Fehler-Taxonomie (Punkt A) — eine Ebene tiefer, keine Interceptor-Klasse**
- `TrameErrorCodes` (neu) ersetzt Magic Numbers in `TrameResults` und Invoker-Fabriken.
- `TrameError.Category` (neu, optional) — semantische Kategorie (`InvalidArgument`/`NotFound`/
...
- `TrameResults` und Invoker-Fabriken werden *gemeinsam* auf `TrameErrorCodes` +
...
1. **`TrameOptions.Interceptors` + `TrameOptions.BatchInterceptors` Collections** + DI-Wiring.
   Default: die drei Built-ins (Auth/Telemetry/Logging) in fester Reihenfolge, User-Interceptors
   anhängbar.
2. **`TrameInvocationContext` erweitern** (s.o.), `ITrameInterceptor.InvokeAsync` darauf
   umstellen, `ITrameBatchInterceptor` neu.
...
4. **Fehler-Fabriken vereinheitlichen:** `TrameErrorCodes` + `TrameError.Category`, `TrameResults`
...
### Entscheidung 1 — `ITrameInterceptor`-Signatur-Break: **ja**

Heute `InvokeAsync(TrameRequest, TrameInvocationDelegate, CancellationToken)`. Umstellung auf
`InvokeAsync(TrameInvocationContext, Func<TrameInvocationContext, Task<TrameResponse?>>)`. Das ist
ein **breaking change** für jeden, der `ITrameInterceptor` heute implementiert (im Repo nur
`TrameLoggingInterceptor`, aber potenziell externe User). `STABILITY.md` §2 markiert
`ITrameInterceptor` als **experimental** — also ist der Break innerhalb 1.x erlaubt. Der
einzige Built-in (`TrameLoggingInterceptor`) wird mit umgestellt.

### Entscheidung 2 — `TrameError.Category` als additives Feld: **ja**

`TrameError.Category` (neu, optional) kommt *neu* hinzu. Das ist eine `TrameError`-Schema-
...
`[TrameAuthorise(Policy = "CanApproveOrder")]` ruft
`IAuthorizationService.AuthorizeAsync(user, resource, policy)` auf. Da Trame command-orientiert
ist und Controller-Methoden keinen klaren "resource"-Begriff haben: **(a) `resource = null` für
v1.1** (nur Policy-Check, kein resource-basierter). Ein `[TrameAuthorizeResource]`-Hook für
resource-basierte Policies ist **(c) v1.x+** — nicht Teil von Phase 1.

### Entscheidung 4 — Meter-Name `"Trame"`

`Meter` und `ActivitySource` tragen denselben Namen `"Trame"` (OTel-Konvention erlaubt das).
`TrameTracing.ActivitySourceName = "Trame"` bleibt; neuer `TrameMetrics.MeterName = "Trame"`.
...
- **Keine Änderung am Tracing-Span-Modell.** Der `TrameCall`-Span bleibt, wie er ist (umschließt
...
- [ ] `trame.*`-Metrics in OTLP (`Meter "Trame"`)
- [ ] `ERROR_CATALOG.md` mit stabilen Codes + `TrameError.Category`
- [ ] JSON-RPC-Mapping auf `Category` statt String-Präfix
- [ ] Eine Fehler-Factory statt zwei (`TrameResults` + Invoker-Fabriken vereinheitlicht)
- [ ] Batch-Pfad läuft durch die Interceptor-Pipeline (Single- und Batch-Pfad kohärent)
- [ ] `TrameInvocationContext` mit `HttpContext`, `InvokeInfo`, `Response`, `Activity`
- [ ] `STABILITY.md`-Updates: Auth/Telemetry/Fehler-Interceptors von experimental → stable (nach
      Landung), `TrameError.Category` als additives Feld dokumentiert

---

## Implementierungs-Reihenfolge

1. `TrameOptions.Interceptors`/`.BatchInterceptors` Collections + DI-Wiring (Schritt 1).
2. `TrameInvocationContext` erweitern + `ITrameInterceptor`-Signatur umstellen + `ITrameBatchInterceptor` neu (Schritt 2).
3. `TrameLoggingInterceptor` auf neue Signatur migrieren (zusammen mit Schritt 2, damit die
   Pipeline nicht kaputtgeht).
4. `TrameErrorCodes` + `TrameError.Category` + Fehler-Fabriken vereinheitlichen (Schritt 4).
5. `TrameAuthorizationInterceptor` (Policies) + Batch-Pre-Pass auf denselben Interceptor umstellen (Schritt 3/Auth-Teil).
6. `TrameTelemetryInterceptor` (Metrics + Logging-Conventions) + acht `TrameTracing`-Stellen konsolidieren (Schritt 3/OTel-Teil).
