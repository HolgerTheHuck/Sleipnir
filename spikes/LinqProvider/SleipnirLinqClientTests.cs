...
using Sleipnir.Spike.LinqProvider;
...
namespace Sleipnir.Spike.LinqProvider.Tests;
...
/// </summary>
public class SleipnirLinqClientTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    public SleipnirLinqClientTests(WebApplicationFactory<Program> factory)
    {
        // Die Sample-App registriert [SleipnirController]-Typen (z. B. CustomerHandler)
        // per Auto-Scan in AddSleipnir — kein explizites Register nötig.
        _factory = factory;
    }

    private SleipnirLinqClient CreateClient()
    {
        // SleipnirRestJsonClient baut absolute URIs aus serverBaseUrl. Damit das
        ...
        var restClient = new SleipnirRestJsonClient(baseAddr, http);
        return new SleipnirLinqClient(restClient);
    }
    ...
    public async Task Batch_mit_StringDep_ueber_PropertyPfad_liefert_Wert_intakt()
    {
        ...
        var batch = new SleipnirBatch(create, fetch, readd);
    ...
    public void ContractGenerator_erzeugt_Vertrag_fuer_Customer_aus_Discovery()
    {
        ...
        source.Should().Contain("[SleipnirServiceContract(\"Customer\")]");
        source.Should().Contain("interface ICustomerService");
        source.Should().Contain("[SleipnirMethodContract(\"AddCustomer\")]");
        source.Should().Contain("Task<int?> AddCustomer(Arg<string> name);");
        source.Should().Contain("[SleipnirMethodContract(\"GetCustomerById\")]");
        source.Should().Contain("Task<Customer?> GetCustomerById(Arg<int> id);");
        source.Should().Contain("[SleipnirMethodContract(\"UpdateCustomerName\")]");
