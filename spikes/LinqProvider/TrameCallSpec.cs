...
/// <see cref="TrameCallSpec{T}.Expose()"/> / <see cref="TrameCallSpec{T}.Expose{TProp}"/>
...
/// <summary>
/// Ein typsicher gebauter Trame-Call mit Rückgabetyp <typeparamref name="T"/>.
/// Entsteht aus <see cref="TrameLinqClient.Build{T}"/> /
/// <see cref="TrameLinqClient.BuildVoid{TService}"/>. Über <c>Expose</c> werden
/// <see cref="Dep{T}"/>-Marker für Folge-Calls bereitgestellt. Der gemeinsame
/// Zustand liegt in der Basisklasse <see cref="TrameCallSpec"/>.
/// </summary>
public sealed class TrameCallSpec<T> : TrameCallSpec
{
    internal TrameCallSpec(string controller, string method, string id, JsonNode? paramsNode)
    ...
    /// Der JsonPath ist ergebnisrelativ (siehe Trame-Konvention).
    /// </summary>
