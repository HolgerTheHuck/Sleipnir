...
// The generated client imports `from "sleipnir-client"`; alias it to the runtime
...
export default defineConfig({
  // Served by the Story-01 API itself at /story01 (same origin as /api/sleipnir/*), so the
  // first walkthrough needs no Vite dev server, no proxy, and no CORS. The built bundle's
  // asset paths are rooted at /story01/ to match. `new SleipnirClient("/")` in main.ts still
  // issues same-origin relative /api/sleipnir/json calls (the API lives at the host root, not
  // under /story01).
  base: "/story01/",
  resolve: {
    alias: {
      "sleipnir-client": resolve(here, "../../../clients/ts/src/index.ts"),
    },
  },
