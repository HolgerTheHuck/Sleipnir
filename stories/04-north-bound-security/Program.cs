...
//
//    The hardened server. Sleipnir was south-bound (trusted caller); this is it
...
//
//    In Visual Studio: open Story04.sln, press F5 → browser lands on /Sleipnir. The
//    DevUI loads but its discovery call 401s — that IS the lesson. Enter the demo
//    token in the DevUI Auth panel (sleipnir-demo) and the contract appears. The
...
// Demo-Auth-Middleware VOR den Sleipnir-Transporten: belegt HttpContext.User, den
// der Invoker (CheckAuthorisation) und die Transporte (WS-Upgrade/Hub-Endpoint)
// lesen. Produktion: hier steht echtes JWT/Cookie/mTLS — Sleipnir selbst misst nichts.
app.UseMiddleware<DemoAuthMiddleware>();

app.UseStaticFiles();
app.UseRouting();
app.UseRateLimiter();        // aktiviert die "sleipnir"-Policy (RateLimitPermitLimit>0)
app.UseSleipnirTransports();    // WS-Upgrade gate-t auf RequireAuthentication; REST via Invoker
app.MapSleipnir();              // REST-Discovery gate-t; SignalR-Hub .RequireAuthorization()
